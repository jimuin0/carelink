...
test('オーナーがスタッフを編集できる（作成→編集→一覧反映）', async ({ page }) => {
  ...
  // PATCH /api/admin/staff/{id} の 2xx を成功シグナルに（click より前に登録）。揮発トースト依存を外し、
  // 下の一覧再訪（edited 反映・original 消滅＝永続 DOM）で確定する。
  const staffPatch = page.waitForResponse(
    (r) => /\/api\/admin\/staff\/[^/?]+/.test(r.url()) && r.request().method() === 'PATCH' && r.ok(),
    { timeout: 15000 }
  );
  await page.getByRole('button', { name: '保存', exact: true }).click();
  await staffPatch;

  // 一覧へ戻り、編集後の名前が反映され旧名は消えていること（書き込み永続化＋再読込反映）。
