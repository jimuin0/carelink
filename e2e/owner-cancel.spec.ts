...
test.describe.serial('管理画面（オーナー）キャンセル／無断キャンセル', () => {
  ...
  test('承認待ち予約をオーナーがお断り（キャンセル）できる', async ({ page }) => {
    ...
    // 破壊的操作（顧客へキャンセル通知メールが飛ぶ）は確認ダイアログを挟む（誤タップ事故防止）。確認して実行。
  ...
  test('確定予約をオーナーが無断キャンセルにできる', async ({ page }) => {
    ...
    // 確認ダイアログで実行を確定（無断キャンセル通知を伴う破壊的遷移）。
    await page.getByRole('button', { name: '無断キャンセルにする', exact: true }).click();
