...
test('オーナーがクーポンを作成→編集→削除できる（書き込み→一覧反映/消滅）', async ({ page }) => {
  ...
  // PATCH /api/admin/coupons/{id} の 2xx を成功シグナルに（press より前に登録）。編集は遷移せず
  // 揮発トーストが唯一証拠だったため、API 2xx を主判定にし、下の一覧再訪（永続 DOM）で確定する。
  const couponPatch = page.waitForResponse(
    (r) => /\/api\/admin\/coupons\/[^/?]+/.test(r.url()) && r.request().method() === 'PATCH' && r.ok(),
    { timeout: 15000 }
  );
