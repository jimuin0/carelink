...
test('オーナーがメニューを作成→編集→削除できる（書き込み→一覧反映/消滅）', async ({ page }) => {
  ...
  // POST /api/admin/menus → 成功トースト＋一覧反映（書き込み永続化）。
  ...
  // PATCH /api/admin/menus/{id} → 成功トースト＋一覧に編集後の名前が反映（旧名は消える）。
  ...
  await expect(confirm.getByText('メニューを削除')).toBeVisible();
  ...
  // DELETE /api/admin/menus/{id} → 成功トースト＋一覧から消滅（永続削除）。
