...
test('オーナーが顧客台帳の顧客を編集できる（書き込み→反映）', async ({ page }) => {
  ...
  // PATCH /api/admin/customers/{id} の 2xx を成功シグナルに（press より前に登録）。揮発トースト依存を
  // 外し、下の一覧反映（editedName 可視＝永続 DOM）で確定する。
  const custPatch = page.waitForResponse(
    (r) => /\/api\/admin\/customers\/[^/?]+/.test(r.url()) && r.request().method() === 'PATCH' && r.ok(),
    { timeout: 15000 }
  );
