...
- `20260605_booking_menu_ids.sql`（bookings.menu_ids・複数メニュー保存）→ 適用完了。複数メニュー予約が
  全件保存・全層表示されるように。
- `20260607_get_month_availability.sql`（#4・availability 集約RPC）→ 適用完了。月カレンダーが 310往復→1往復。
- `20260607_recommended_indexes.sql`（#5・menus/photos sort_order・reviews created_at 複合索引）→ 適用完了。
...
### Gmail メール正規化（2026-06-04・神原さん「Gmail を使う人が圧倒的に多い」）

🔴 **確定バグ（金銭・発症前修正）**: 顧客識別を生 email キーで行っていたため、Gmail の
ドット無視（f.o.o=foo）・"+tag"無視・googlemail=gmail を悪用/誤認し、**ゲストが別エイリアスで
new_customer 限定クーポンを複数回取得**（金銭被害）・repeat 誤拒否・RFM/customer_segments 分裂が起き得た。

**採用方式: email_canonical 列方式（神原さん選択(P)・副作用ゼロ）**。当初は保存 email 自体を canonical 化したが
（commit `2935f99`）、Gmail パワーユーザーの "+tag"（届け先/フィルタ）を失う UX 副作用があったため、神原さんの
敵対検証指摘を受け方式変更:
- **保存・送信・表示は原文（小文字化のみ）**を保持（"+tag"・ドットを届け先として尊重）。
- **突合（new_customer/repeat 履歴照合・RFM 集計・台帳）は `email_canonical` で行う**。`email_canonical` は
  **Postgres GENERATED ALWAYS AS ... STORED 列**で email から自動算出（`canonicalizeEmail` と同一出力）＝
  DB が常に一貫性保証・列追加時に既存行も自動算出（別 backfill 不要）・直接書込不可でドリフトしない。
- 入力側は `lib/email-canonical.ts` の `canonicalizeEmail`（gmail/googlemail のみドット/"+tag"除去・ドメイン統一／
  非Gmailは小文字化のみ＝別人誤併合を回避）で照合値を算出。
- 適用箇所: `bookingSchema.email` は toLowerCase に戻し、`booking/route.ts` の new_customer/repeat 照合と
  `customer-segment` RFM 集計と `lib/admin.ts` 台帳集計を email_canonical 化。
- email-canonical 100%（branch/stmt/func/line）・admin/customer-segment/booking 全 branch100%維持・L5プロパティ・
  全5085テスト通過。
- **🔴 適用待ち migration**: `20260607_email_canonical_column.sql`（bookings/customer_visits に email_canonical
  生成列＋複合索引・冪等/非破壊）。旧 `20260607_canonicalize_gmail_emails.sql` は撤回（email 自体を変えない方式に変更）。

**(P+) デプロイ順序非依存フォールバック（神原さん「壊れないように慎重に」）**: email_canonical 列が
未適用(migration前)の環境でコードが出ても壊れないよう、`isMissingColumnError` で列不在を検知し
**旧挙動（生 email/customer_email 照合）へ自動フォールバック**を実装（真の予防・無破壊・順序非依存）:
- `booking/route.ts` ゲスト履歴照合: email_canonical 不在→生 email 照合
- `customer-segment/route.ts` RFM: email_canonical 不在→email 取得＋JS canonicalize 集計
- `lib/admin.ts` 台帳: email_canonical 不在→customer_email/JS canonicalize
全フォールバック分岐に branch100% テスト追加（admin/customer-segment 100%・booking 該当分岐被覆）。
→ migration とコードのデプロイ順序がどちらでも本番は壊れない（適用後は email_canonical 経路が有効）。

**現在地（feat/salon-board・main統合後）:** 全210スイート/5090テスト通過・tsc/eslint エラー0・本番ビルド891/891。
