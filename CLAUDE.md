...
- 定期実行＝GitHub Actions cron（`.github/workflows/cron.yml`・13 ジョブ）が `/api/cron/*` を Bearer 認証で叩く。月次の `newsletter-digest` は別ワークフロー（`newsletter-digest.yml`・self-heal）
...
月次の `/api/cron/newsletter-digest` は cron.yml ではなく専用ワークフロー `newsletter-digest.yml`（`0 17 1-7 * *`＝毎月1〜7日 JST 02:00 に self-heal 再試行）で起動し、per-email 台帳 `newsletter_send_log` ＋決定的 idempotency key で「当月ちょうど1回」を保証する。`monthly-batch-watcher.yml` がその月次発火を監視する。
...
- 他ワークフロー：`mutation-l4.yml`（Stryker）・`health-monitor.yml`（外形監視）・`cron-constraints.yml` / `anon-write-policy-lint.yml` / `secdef-search-path-lint.yml` / `actionlint.yml`（静的ガード）・`deploy-watch.yml` / `vercel-preview-build.yml` / `dependency-update.yml`（依存更新）・`newsletter-digest.yml`（月次配信 self-heal）・`monthly-batch-watcher.yml`（月次発火監視）
