...
🔴 **withRoute を使わない route.ts でも、CSRF・レート制限・本人確認は `src/__tests__/route-guard-coverage.test.ts` が機械強制する**（2026年8月22日 新設）。変更系メソッド（POST/PUT/PATCH/DELETE）を持つ route.ts は 3 点すべてを満たすか、同テストの `ALLOW` に【理由つき】で登録されていなければ CI が落ちる。理由の空欄も、消したルートの免除が残っているのも落ちる。免除して良いのは webhook（署名検証が本人性の根拠）・未ログインで使う公開フォーム・処理本体の無いスタブだけ。
...
### 配色テーマ＝`.theme-ecru`（`src/app/globals.css` が単一ソース・2026年8月18日 新設）

施設向けの掲載登録ページ（`/register`）で使うくすみカラー（韓国系のトーン）。
**色コードを持ってよいのは `globals.css` の `.theme-ecru` だけ。**使用側は
`bg-[var(--ecru-bg)]` のように変数を参照する。

| 役割 | トークン | 色 | 実測コントラスト |
|---|---|---|---|
| 地色（エクリュ） | `--ecru-bg` | `#F7F3EC` | — |
| 面（アイボリー） | `--ecru-surface` | `#FFFCF9` | — |
| 罫線（ベージュ） | `--ecru-line` | `#E6DCCD` | — |
| 本文 | `--ecru-text` | `#4A4038` | 地色に 9.12:1 ／ 面に 9.87:1 |
| 副文 | `--ecru-muted` | `#6F655C` | 5.14:1 ／ 5.56:1 |
| アクセント（くすみローズブラウン） | `--ecru-accent` | `#8E6A61` | 白文字 4.79:1 ／ 地色の上 4.33:1 |
| 写真に重ねる覆い | `--ecru-scrim` | `#3A302A` | — |

🔴 **色は目で選ばず、コントラスト比を計算してから決めること。** この repo は axe の a11y 検査
（`e2e/a11y-audit.spec.ts`）を持っており、薄すぎる文字は CI で落ちる。候補に挙がった
`#7D7268`（4.24:1）と `#9C9086`（2.81:1）は AA（4.5:1）に届かないので**本文には使わない**（罫線のみ）。
ロゴの `#8E6A61` は地色の上で 4.33:1 だが、24px 太字＝大文字扱いで基準 3:1 を満たす。

🔴 **`.theme-ecru` は `--primary` / `--primary-dark` も束ねている。** `.btn-primary`・
`StepIndicator`・focus リングは全て `var(--primary)` を見ているため、テーマを当てるだけで
フォーム側も同じ色に寄る。個々のボタンへ `!important` を撒くと `disabled` の灰色まで潰れる。

🔴 **ヘッダーはページの器の外（レイアウト側）にある。** ページ内で変数を上書きしても届かず、
実際に**ロゴだけ濃い青が残ってアイボリーの面から浮いた**。`Header.tsx` が
`pathname === '/register'` を見て同じクラスを付ける形にしてある。
`src/__tests__/ecru-theme-single-source.test.ts` が「単一ソースの実在」「使用側に色コードの
直書きが無いこと」「ヘッダーの配線」を CI で固定する（直書きへ戻すと落ちる）。

⚠️ サイト全体のブランド色（`:root` の `--primary: #0369A1`）は変えていない。
トップ・検索など他のページは従来どおり。
...
- 【🔴 Next.js は 16.3.0 以上を維持する。15.5 系には「子→親のクライアント遷移が RSC ペイロード約29KB 超で【無音失敗】する」不具合がある】2026年8月13日に実測で特定。`/admin/jobs/new` や `/admin/jobs/applications` から一覧 `/admin/jobs` へ戻る `<Link>` / `router.push` が、**URL も画面内容も変わらず、JS エラーも出ないまま何も起きない**。ユーザーには「押しても反応しない」としか見えない。`new` と `[id]/edit` を持つ管理画面（求人・ブログ・カタログ・クーポン・スタッフ・プラットフォームブログ）はすべて同じ構造なので、掲載件数が増えた施設で必ず発症する。求人作成では遷移しないためユーザーが再送信し、**POST は毎回成功するので重複した求人が作られる**（ローカル再現で32件積み上がるのを DB で実測）。
  - **A/B/A 対照で確定**：同一コード・同一データで Next のみ入れ替え。15.5.21 🔴 / 15.5.23（15系最新パッチ）🔴 / **16.3.0 ✅**（`/admin/blog` 58,543B・`/admin/jobs` 40,493B とも 200〜250ms で遷移）。閾値は求人を1件ずつ増やす二分探索で **28,687B 成功 / 30,641B 失敗**。16.3.0 では **455,229B（旧閾値の14.9倍）でも 336ms で成功**＝閾値が上がっただけではない。
  - **無実を確認した要素**（すべて対照実験で否定）：middleware・プリフェッチ・`router.refresh()`・POST/フォーム/react-hook-form・サーバ描画（直接ロードは正常）・ネットワーク（素の `fetch` は全バイト読める）。サブツリー外（`/admin/menus → /admin/jobs`）は 37ms で成功する。
  - **機械強制**：`src/__tests__/next-version-floor.test.ts` が package.json の要求と **実際にインストールされた版**の両方を検査（宣言と実体の乖離を防ぐ）。挙動側は `e2e/admin-large-list-nav.spec.ts` が、閾値を大きく超えるデータを seed したうえで**ペイロードが本当に閾値を超えていることを測ってから**遷移を検証する（seed が効かなくなった時に緑のまま素通りしない）。両方とも負の対照つきで、Next 15 へ戻すと実際に落ちることを実測済み。
  - ⚠️ **`admin-jobs.spec.ts` だけでは検出できない**。求人を1件しか作らずペイロードが閾値の下に収まるため、不具合があっても通る回がある（実際 CI で3回中1回だけ通り原因究明を遅らせた）。
  - ⚠️ **Next 16 で `next lint` は削除された**。`npm run lint` は `eslint --ext .js,.jsx,.ts,.tsx src` を使う（`next lint` に戻すと `next <dir>` と解釈され "Invalid project directory" で CI が落ちる）。移行時に両者の出力が **75 warning・0 error でファイル/行/列/規則まで完全一致**することを機械比較で確認済み。
  - ⚠️ Next 16 のビルドは `tsconfig.json` を自動更新する（`jsx: preserve → react-jsx`、`.next/dev/types` の include 追加）。差分が出ても異常ではない。React の更新は不要（16.3 は React ^18.2 を許容）。
- 【🔴 Next 16 は viewport に入った `<Link>` の【動的ルートのデータまで】先読みする。一覧を持つ画面はそのまま上げると「表示しただけで」SSR が十数回走る】2026年8月14日に実測。同一コード・同一データで Next の版だけを入れ替え、トップ `/` の load イベント以降に開始した通信を計測：**15.5.23 = 0 本 / 16.3.0 = 85 本（うち RSC ページ先読み 20 本）**。CareLink のトップは業種・地方・47都道府県・主要都市・こだわり条件と「数で広がる」導線を持つため、`/search`（ビルド出力 `ƒ`＝オンデマンド SSR・実 DB クエリ）のレンダリングが**閲覧 1 回につき十数回**走っていた。モバイルでは `/mypage/*`（middleware の保護対象＝先読みのたびに `auth.getUser()`）まで先読みされていた。
  - **E2E がこれを正しく検出して落ちた**（`accessibility.spec.ts` の `waitForLoadState('networkidle')` が 30 秒で settle しない）。テストの過敏ではないので `networkidle` を弱める方向で消さないこと。
  - 対処＝`src/components/BrowseLink.tsx`（`prefetch={false}` を既定にした `next/link` の薄いラッパ）を作り、一覧・回遊リンクを描くコンポーネントで `import Link from '@/components/BrowseLink'` に差し替えた（JSX は無変更）。**実測 20 → 4 本**。高意図の導線（登録 CTA 等）は `next/link` のまま先読みさせている。
  - ⚠️ `next.config.mjs` の `partialPrefetching: true` でも同種の効果が得られるが、**`cacheComponents: true`（PPR/`use cache` への全面移行）が前提**なので採らなかった。
  - 機械強制＝`e2e/home-prefetch-fanout.spec.ts`。個々のリンクに `prefetch={false}` が書いてあるかは**見ない**（列挙は次に足される一覧を守らない）。**トップ全体で何本のページ先読みが出たか**という外形だけを見るので、新しい一覧を足して先読みが復活すれば何であっても赤くなる。空振り防止として「計測機構が要求を観測したか」と「一覧リンクが実際に描画されたか」を併設してある。
  - ⚠️ **ローカル計測の偽陰性に 2 回踏んだ。** `.next` を消して再ビルドしても**古い `next start` が生き残っている**と、消えたチャンクを配って 500 になり、`ssr:false` の `HomeBelowFold` が描画されないまま「先読み 0 本」に見える。`pkill -f "next start"` は**自分のシェルにもマッチして落ちる**ので使わない。`/proc` の cmdline で `next-server` を特定して落とし、`お役立ちコラム` 等が描画されていることを確認してから数えること。
...
- 【🔴 RLS は列を絞れない。「その施設のメンバーか」だけのポリシーは全列の書き換えを許す】`facility_reviews_member_update` は施設メンバーであることしか見ておらず、管理画面が実際に使うのは `status`（公開／非表示）だけなのに、PostgREST を直に叩けば **客が書いた `rating` と `comment` まで書き換えられた**（集計トリガが平均点を再計算するので、星の書き換えはそのまま評価点の水増しになる）。列を絞る手段は RLS ではなく【列レベル GRANT】しか無い（`GRANT UPDATE (status) ON facility_reviews TO authenticated`）。2026年8月22日の migration `20260822000001` で是正済み。同じ形の穴として `reports` は「どこかの施設のオーナーであること」しか見ておらず、**全施設の通報を通報者の IP つきで読め、削除もできた**（アプリ側に読み取り経路は 1 本も無いので service_role 専用へ落とした）。新しいポリシーを書くときは「行を絞る条件」と「触ってよい列」を別々に考えること。
- 【🔴 RLS は【書き込みポリシーの不在】も無音の機能欠落になる】`review_replies` は公開読みポリシー 1 本だけで INSERT ポリシーが無く、管理画面の口コミ返信（ブラウザの authenticated クライアントから INSERT）は**押すたびに必ず失敗していた**。RLS 有効かつ該当コマンドのポリシー不在＝常に拒否。UI が既にある機能を足すときは、読みだけでなく**そのコマンドのポリシーがあるか**を `src/lib/schema-fingerprint.expected.json` の `policy|<table>|` 行で確認する。
- 【🔴 SECURITY INVOKER のトリガが RLS のある表を UPDATE すると 0 行更新で無音停止する】`update_facility_rating()` は SECURITY INVOKER のまま `facility_profiles` を UPDATE していた。`facility_profiles` には SELECT ポリシーしか無いので、authenticated 文脈（＝管理画面から口コミを非表示にしたとき）に走ると **例外も出ずに 0 行更新**となり、平均点・件数が置いていかれた。service_role 経由（/api/review）では RLS を素通りするため発症せず、管理画面のときだけズレるので気づけない。集計トリガのように「呼び出し元の可視性に依存してはいけない」ものは SECURITY DEFINER ＋ `SET search_path` にする。
...
- 【🔴 `facility_card_view` には「テーブルには在るが view には無い列」がある。参照すると PostgREST 400 → error 握り潰しで無音全滅する】かつて `nearest_station` が view 非射影のままキーワード検索の `.or()` から参照され、**例外もログも出ないままキーワード検索が常に0件**になっていた（migration `20260722000002` で射影して解消）。✅ **2026年8月20日訂正**: `<Database>` 型は各 Supabase クライアントへ**配線済み**（`supabase-server.ts` / `supabase-server-auth.ts` / `supabase-browser.ts` が `src/types/database-overrides.ts` の `Database` を渡している）。`facility_card_view` も型に載っているため、**view に無い列の参照は `tsc` が検知する**。以前ここに書かれていた「配線されていないので tsc では防げない」は現状と合っていない。ただし型は本番 introspection の生成物なので、**型を再生成する前に**view へ列を足す作業をすると型が追いつかない。view に列を足す前に `src/lib/schema-snapshot.json` の `facility_card_view` を見ること。`src/__tests__/card-view-columns.test.ts` が select 列・`ilike` 対象列・並び替え列を実在列と突合して CI で止める。
- 【存在しないテーブル名・列名を参照して無音停止する事故が繰り返し発生している】（例：`menus`→正しくは`facility_menus`、`reviews`→正しくは`facility_reviews`、`facility_menus.is_active`のように元々存在しない列）。✅ **2026年8月20日訂正**: `<Database>` 型は配線済みで、**存在しない列の参照は `tsc` が検知する**（`.update({不明な列})` は TS2353、`.select('不明な列')` の結果参照は TS2339）。以前ここに書かれていた「型付けされていないため検知できない」は現状と合っていない。🔴 **ただし型の更新順序を間違えると詰まる**: 新しい列を使うコードを先に書くと `tsc` が落ちるので、**migration →`src/types/database.types.ts` →`node scripts/gen-schema-snapshot.mjs` →`scripts/gen-schema-fingerprint.sh` →コード** の順で進めること。`database.types.ts` の完全な再生成には `supabase login` が要るが、**列の追記だけなら手で書ける**（`gen-schema-snapshot.mjs` は types を読むだけで DB へ接続しない）。新しいテーブル／列を参照する前に必ず `src/lib/schema-snapshot.json`（実在テーブル／列の正）で実在を確認する。`schema-drift-check` cron は事後検知であり事前予防にはならない。
