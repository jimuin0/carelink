...
**🔴 適用待ちマイグレーション 3件（神原さん・Supabase SQL Editor・冪等/非破壊）:**
...
3. `20260606_booking_occupancy_authoritative.sql`（占有判定RPC権威版の再適用。本番は適用済みのため任意・推奨）

→ 本番監査で見つかった確定バグ（🔴6・🟡6）は全て根本修正済み。全4976テスト/208スイート通過。
