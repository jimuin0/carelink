...
| L4 | Stryker ミューテーション | ✅ | agent1 4ソース（i18n / seo-constants / seo-snippets / json-ld）Survived=0 を Stryker 公式実行で確定（2026-05-31）。高負荷下のOOM kill回避のため8分割並列＋順次リトライで完走。seo-snippets.ts の生存1体（`.slice(0,180)` 削除）は到達不能な防御コードに起因する等価変異だったため、180字上限を純粋関数 `truncateText`＋定数 `INTRO_MAX_LENGTH` に抽出し境界テストで kill 可能化（症状抑止ではなく予防的根本解決）。変更範囲 Stryker 再実行で Mutation score 100.00 確認。stryker.config.mjs の mutate は純粋10モジュール（上記4＋constants/safe/image-utils/jobs/validations/validations-booking/validations-auth）を break:100 で列挙済み（ただし上記4以外は未検証＝下記）。**【2026-06-10 訂正・要再検証】**: `validations.ts`（124 mutant）の L4 を試みたが**実 Mutation score は未確認**。当初「exit 0＝100%確定」と記載したが、その exit 0 は実は Stryker の TS チェッカーが **stale な `.next/types`（main に無い feat 由来ルートを参照）でクラッシュ**したもので、実スコアは取得できていなかった（誤記を訂正）。TS チェッカーを外した config では dry-run 成功・124 mutant のミューテーションは走るが、jest の per-mutant overhead が約45秒で **124 mutant×2並行≒46分**かかり当セッションでは完走不可だった。**L4 を正しく検証するには (a) `.next` を再生成して stale 参照を解消、(b) per-mutant overhead を下げる（jest --cache 等）、(c) 1ファイルずつ非並行（並行/多ファイルは OOM kill を再現確認）で実行すること。validations.ts 等 上記4以外の実 Mutation score は未確認＝L4 完了とは言えない。** |
