...
- 定期実行＝GitHub Actions cron（`.github/workflows/cron.yml`・13 ジョブ）が `/api/cron/*` を Bearer 認証で叩く。オーナーニュースレターは月次自動配信を停止し（神原さん確定 2026年7月2日「お知らせがある時のみ」）、`newsletter-digest.yml`・`monthly-batch-watcher.yml` はともに `workflow_dispatch` 手動専用。日常配信は管理画面 `/admin/newsletters` から手動送信
...
`/api/cron/newsletter-digest`（自動生成の月次サマリー digest）は、月次自動配信を停止し（神原さん確定 2026年7月2日）専用ワークフロー `newsletter-digest.yml` を `workflow_dispatch` 手動専用にした。必要な月だけ手動実行でき、per-email 台帳 `newsletter_send_log` ＋決定的 idempotency key で「対象月ちょうど1回」を保証する（手動で複数回叩いても二重送信なし）。監視用 `monthly-batch-watcher.yml` も、監視対象の月次発火が無くなったため `schedule` を撤去し手動専用（偽陽性の未発火アラート防止）。日常のお知らせ配信は管理画面 `/admin/newsletters` で任意の件名・本文を作成し「今すぐ配信」する。
