...
1. `20260604_facility_members_owner_unique.sql`（owner 部分ユニーク・setup TOCTOU 防御）
2. `20260605_booking_menu_ids.sql`（bookings.menu_ids・複数メニュー保存）

**🟡 残（確定・順次修正中）:** register 住所3分割不整合 / リマインダー .limit(200)＋confirmedのみ＋
メニュー担当名なし / 誕生日・お気に入りダイジェスト .limit(500)＋配信停止無視 / マイページ取得失敗の
「データなし」誤表示 / payjp/charge の二重課金窓 / create_booking_atomic の再定義順(fresh db push 退行リスク)。
