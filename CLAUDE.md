...
- 真の予防: `lib/email-canonical.ts` の `canonicalizeEmail`（gmail/googlemail のみドット除去・"+tag"除去・
  ドメイン統一／非Gmailは小文字化のみ＝別人誤併合を回避）を新設し、`bookingSchema.email` の保存値を
  canonical 化。保存=突合が canonical に揃い new_customer/repeat 履歴照合・RFM 集計が正しくなる。
- email-canonical branch/stmt/func/line 100%・L5プロパティ（冪等性/小文字/Gmail不変条件）追加・全5084テスト通過。
- **🔴 適用待ち migration**: `20260607_canonicalize_gmail_emails.sql`（既存 bookings.email / customer_visits.customer_email
  の gmail 行を canonical 化・冪等/非破壊）。既存顧客の旧予約も新予約と突合させるため必須。

**現在地（feat/salon-board・main統合後）:** 全210スイート/5084テスト通過・tsc/eslint エラー0・本番ビルド891/891。
