...
2. 🔴 **shadow と本番の PostgreSQL メジャーバージョンを揃える。** `pg_get_*` の整形は
   ...
   ⚠️ **本番の実測値は未取得**（2026年8月3日時点）。`supabase/config.toml` の
   `major_version = 17`（supabase CLI 規約上リモートと同値であるべき宣言）を根拠に
   CI を 17 系に置いている。セッション中に出た `150001` はどのプロジェクトで実行されたか
   特定できず、同時期の `170006` は soel（別プロジェクト）だった。食い違っていれば
