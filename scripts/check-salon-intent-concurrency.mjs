...
SELECT outcome || '|' || receipt_id::text FROM public.commit_salon_submission(
  '${intent}', repeat('a',64), 1::smallint, 'proof-hkdf-sha256-v1', repeat('b',64),
...
SELECT id FROM public.salon_submission_intents WHERE id='${intent}' FOR UPDATE;
...
COMMIT;`);
  ...
  const counts = execFileSync('psql', args, { env: dbEnv, encoding: 'utf8', timeout: 30000,
    stdio: ['pipe', 'pipe', 'pipe'], input: `SELECT
      ...
      (SELECT count(*) FROM public.salon_submission_intents WHERE id='${intent}' AND salon_id IS NOT NULL)::text;` }).trim();
  if (counts !== '1|2|1') throw new Error('atomic receipt/outbox count mismatch');
  console.log('Registration concurrency passed: 20 lock-waiting calls, 1 receipt, 2 distinct logical notifications. Disposable CI data only.');
}
