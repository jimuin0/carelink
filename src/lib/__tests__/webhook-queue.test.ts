...
test('scheduleRetry: attempt=1 → pending状態に再スケジュール', async () => {
  const updateMock = jest.fn().mockReturnValue({ eq: jest.fn(() => Promise.resolve({ error: null })) });
  mockFrom.mockReturnValue({ update: updateMock });
...
test('scheduleRetry: attempt=2 → scheduled_at が30分後', async () => {
  ...
  // Should be ~30 minutes in the future (give 1-second tolerance)
