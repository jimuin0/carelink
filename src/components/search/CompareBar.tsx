...
export default function CompareBar() {
  ...
  return (
    <div className="fixed bottom-20 left-1/2 -translate-x-1/2 z-40 bg-white shadow-lg rounded-full border border-gray-200 px-4 py-2 flex items-center gap-3">
      ...
      <Link href={`/compare?ids=${getCompareList().join(',')}`} className="text-sm font-bold text-white bg-sky-600 px-4 py-1.5 rounded-full hover:bg-sky-700 transition-colors">
        比較する
      </Link>
      <button
        type="button"
