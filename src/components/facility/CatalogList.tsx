            ...
            <div className="flex">
              {catalog.before_photo_url && (
                <div className="relative flex-1 aspect-square">
                  <Image src={catalog.before_photo_url} alt="Before" fill className="object-cover" sizes="25vw" />
                  <span className="absolute bottom-1 left-1 bg-black/50 text-white text-[10px] px-1.5 py-0.5 rounded">Before</span>
                </div>
              )}
              {catalog.after_photo_url && (
                <div className="relative flex-1 aspect-square">
                  <Image src={catalog.after_photo_url} alt="After" fill className="object-cover" sizes="25vw" />
                  <span className="absolute bottom-1 left-1 bg-sky-500/80 text-white text-[10px] px-1.5 py-0.5 rounded">After</span>
                </div>
              )}
            </div>
          )}
            ...
            {catalog.tags && catalog.tags.length > 0 && (
              <div className="flex flex-wrap gap-1 mt-2">
                {catalog.tags.map((tag) => (
                  <span key={tag} className="px-2 py-0.5 bg-white border border-gray-200 rounded-full text-[10px] text-gray-500">
                    {tag}
                  </span>
                ))}
              </div>
