  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/40 p-0 sm:p-4" onClick={onClose}>
      <div className="bg-white w-full sm:max-w-md rounded-t-2xl sm:rounded-2xl shadow-xl max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
        ...
        {/* 単一フォーム（メニュー・日時・担当・お客様を1画面に並置。HPB準拠） */}
        <div className="p-5 space-y-4">
          <div><label className={labelCls}>お客様名 <span className="text-red-500">*</span></label><input type="text" value={customerName} onChange={(e) => setCustomerName(e.target.value)} className={inputCls} placeholder="山田 花子" /></div>
          ...
          <div>
            ...
            <select value={menuId} onChange={(e) => handleMenuChange(e.target.value)} className={`${inputCls} bg-white`}>
              <option value="">未選択</option>
              {menuList.map((m) => <option key={m.id} value={m.id}>{m.name}{m.duration_minutes ? `（${m.duration_minutes}分）` : ''}{m.price != null ? ` ¥${m.price.toLocaleString()}` : ''}</option>)}
            </select>
            {(() => { const pr = menuList.find((m) => m.id === menuId)?.price; return pr != null ? <p className="text-xs text-gray-500 mt-1 text-right">料金：¥{pr.toLocaleString()}</p> : null; })()}
          </div>
          <div className="grid grid-cols-2 gap-3">
        ...
        <div className="px-5 py-4 border-t flex items-center gap-3 sticky bottom-0 bg-white">
          ...
          <button type="button" onClick={submit} disabled={saving} className="px-5 py-2 text-sm font-bold text-white bg-sky-500 hover:bg-sky-600 rounded-lg disabled:opacity-50">{saving ? '保存中…' : isEdit ? '更新する' : '登録する'}</button>
        </div>
      </div>
    </div>
  );
