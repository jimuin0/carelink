...
function facilityIdChain(facilityId: string | null) {
  const rows = facilityId ? [{ facility_id: facilityId }] : [];
  return {
    select: jest.fn().mockReturnThis(),
    eq: jest.fn().mockReturnThis(),
    in: jest.fn(() => Promise.resolve({ data: rows })),
  };
}

function multiFacilityIdChain(facilityIds: string[]) {
  return {
    select: jest.fn().mockReturnThis(),
    eq: jest.fn().mockReturnThis(),
...
test('GET: 設定なし → 200 config:null', async () => {
  ...
  mockAdminFrom.mockImplementation(() => {
    callNum++;
    if (callNum === 1) return facilityIdChain(FACILITY_UUID);
...
test('POST: domain が欠落 → 400', async () => {
  mockAdminFrom.mockReturnValue(facilityIdChain(FACILITY_UUID));
...
test('GET: 複数施設所有・facility_id未指定 → 400（従来のlimit(1)非決定選択を排除）', async () => {
  mockAdminFrom.mockReturnValue(multiFacilityIdChain([FACILITY_UUID, FACILITY_UUID_2]));
  const res = await GET(makeGetRequest());
  expect(res.status).toBe(400);
  const json = await res.json();
  expect(json.facilityIds).toEqual([FACILITY_UUID, FACILITY_UUID_2]);
});

test('GET: 複数施設所有・facility_id指定 → 200（指定施設のconfigを返す）', async () => {
  let callNum = 0;
  mockAdminFrom.mockImplementation(() => {
    callNum++;
    if (callNum === 1) return multiFacilityIdChain([FACILITY_UUID, FACILITY_UUID_2]);
    return configSingle({ id: 'wl-multi', domain: 'facility2.example.com' });
  });
  const req = new NextRequest(`http://localhost/api/admin/white-label?facility_id=${FACILITY_UUID_2}`, { method: 'GET' });
  const res = await GET(req);
  expect(res.status).toBe(200);
  const json = await res.json();
  expect(json.config.id).toBe('wl-multi');
});

test('GET: 複数施設所有・所属していないfacility_id指定 → 403（越境防止）', async () => {
  mockAdminFrom.mockReturnValue(multiFacilityIdChain([FACILITY_UUID, FACILITY_UUID_2]));
  const req = new NextRequest('http://localhost/api/admin/white-label?facility_id=99999999-9999-9999-9999-999999999999', { method: 'GET' });
  const res = await GET(req);
  expect(res.status).toBe(403);
});
...
test('POST: 複数施設所有・所属していないfacility_id指定 → 403（越境防止）', async () => {
  mockAdminFrom.mockReturnValue(multiFacilityIdChain([FACILITY_UUID, FACILITY_UUID_2]));
  const res = await POST(makePostRequest({ domain: 'my.salon.jp', facility_id: '99999999-9999-9999-9999-999999999999' }));
  expect(res.status).toBe(403);
});
...
test('POST: 複数施設所有・facility_id指定 → 201（指定施設に保存）', async () => {
  let callNum = 0;
  mockAdminFrom.mockImplementation(() => {
    callNum++;
    if (callNum === 1) return multiFacilityIdChain([FACILITY_UUID, FACILITY_UUID_2]);
    return upsertSingle({ id: 'wl-multi-2', domain: 'my.salon.jp', facility_id: FACILITY_UUID_2 });
  });
  const res = await POST(makePostRequest({ domain: 'my.salon.jp', facility_id: FACILITY_UUID_2 }));
  expect(res.status).toBe(201);
  const json = await res.json();
  expect(json.config.facility_id).toBe(FACILITY_UUID_2);
});
