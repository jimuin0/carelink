...
test('POST: 1000件超は .range() で全ページ取得（10000打ち切り解消）', async () => {
  mockAnonFrom.mockReturnValue(profileChain(true));
  const fullPage = Array.from({ length: 1000 }, (_, i) => ({ id: `${i}`, name: `施設${i}`, created_at: '2026-01-01' }));
  let rangeCall = 0;
  mockAdminFrom.mockReturnValue({
    select: jest.fn().mockReturnValue({
      order: jest.fn().mockReturnValue({
        range: jest.fn(() => { rangeCall++; return Promise.resolve({ data: rangeCall === 1 ? fullPage : [], error: null }); }),
      }),
    }),
  });
  const res = await POST(makePostRequest({ table: 'facility_profiles' }));
