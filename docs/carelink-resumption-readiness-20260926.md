...
### 10.20．W2画面接続と293c4ea7のCI再照合

293c4ea7f2bd71f9c901ef120e947ef31ad141d9を既存PR642へpushし、CI 36243511624でLint／型／Security、local DB Contract、production build／HTTPS E2Eが成功した。Unitは8226成功・1失敗で、失敗はschema-snapshotとproduction由来database.typesの不一致。測定分岐8405/8405は100％だが単体全体は不合格である。hosted Contractも未適用schemaを検知し失敗した。schema-fingerprint run36243511575は期待値差分で失敗したため、その後続fixture／競合工程は未実行。artifact10906563681の完全schema生成候補を期待fingerprintへ厳密一致で取り込む（SHA256 fc9dd1aa9d9730a35ba5b05dba91481fc35b76386c9eff96aaf86bea4d2c796e）。これは本番適用・生成型の代替ではない。

REG-03〜08／13〜16の画面接続を実装中。server flag trueのときだけ署名写真・atomic commitへ切り替える。sessionStorageはタブ単位のintent selector／phaseだけを保持し、申込本文、氏名、email、電話、proof、upload tokenは保存しない。HttpOnly proofはintentごとに分離する。完了画面とonboardingはserverがproofを検証してから選択申込の最小表示項目だけを返すsummary APIで照合する。ログイン／Google OAuthへ渡すURLはhandoff modeだけとし、selectorや個人情報を載せない。別タブで確認メールを開くなどcontextがない場合は、元タブ案内と安全な停止とし、直接施設作成へfallbackしない。

結果不明ではフォームの新規送信を止め、同intentのstatusを先に確認する。committedなら再commitせず受付を表示する。確定前と確認でき、同タブmemoryに送信済の同一bodyがあるときだけ同intentをreplayする。reloadでbodyを失った場合は新しい空formから再構成しない。写真はslot別selection UUIDと圧縮済fileをretryで保持し、upload応答喪失は同manifest／Storage metadata照合で解決する。object削除・overwrite・別intent自動再作成は行わない。

独立レビューを依頼済み。手元のsecret-free／network遮断下でbrowser coordinator異常系19件、実フォームとcoordinatorの結合4件、既存legacyフォーム14件、summary14件を確認した。テストの初回は非表示DOMのlabelで画面遷移を早期判定して3件失敗し、実際の表示buttonを待つよう修正して4件再成功した。新規コードの分岐測定は93.12％でまだ必須100％未達。未測定分岐の検証を追加中であり全体緑とは報告しない。型検査は新規UIを含め成功した。

本番flagは未変更。申込受信実データ、SMTP／OAuth、schema／履歴、本番掲載、全監査台帳の他waveは未確認または未完了のまま維持する。現行1アカウント1owner制約を複数店舗へ変更する経営判断は非同期確認中で、回答前に制約を変更しない。

追加検証で独立レビューのP2（v2の項目別400エラーが汎用toastへ失われる）を修正した。固定allowlist文言をtyped resultで既存のstep／details／focus復帰へ接続。実フォーム＋実coordinatorで前stepのcontact_phoneエラー・任意入力展開・写真slot4の同selection保持を確認した。関連43test成功。coordinator／tab context／summary helper／summary APIの4suite107testは分岐・関数・行100％、statement99.44％へ到達した。未到達だったprivate commitのnull分岐はテストで内部状態を捏造せず、呼出元が必ず持つCommitInputを必須引数にして不要分岐を除去した。既存完了画面、onboarding、receipt、route guard、E2E実行guardの5suite70testも成功。誤った旧テストpathでENOENTとなった呼出しは、実在pathへ訂正して再実行しており、未実行を成功扱いしていない。

追加E2Eは写真slot4の実署名upload、受付確認、合成確認済Authアカウントでの実画面login、選択onboarding、setup201、admin到達、receipt claim／menu写真／draft profileのDB照合を含む。無関係な実メール・本番dataは使わずCI disposable HTTPSだけを許可する。独立再レビューのP3（写真なしという旧コメント）も更新し、最終限定レビューは追加P0〜P3なし。E2E固定hashはdb02764893a7fa8c43cbf0100629166498881854d4b01698bef1dfe81d7926a6。実行成功の証拠は次SHAのCIで取得する。

### 10.21．REG-03管理申込検索の準備計画

現一覧はcreated_at降順の最新100件だけで、古い申込の受信確認ができない。入力は受付UUID／施設名／emailの完全一致検索と審査状態filter、50件単位のkeyset cursorとする。完全一致をUIに明記し、氏名やemailの類似性から同一店舗と判断しない。検索語をbrowser URL／analytics／Refererへ載せないため、新しいPOSTは副作用のない読取としてCSRF・rate・認証・platform admin照合で保護し、GETの既存応答salons配列も維持する。検索語をroute log／エラーに含めない。

cursorはcreated_at降順NULLS LAST＋id降順の一意順序。created_at欠損の既存行も別cursor節で末尾まで列挙し、同時刻の重複・抜けを防ぐ。51行目の存在でnext cursorを返す。strict inputとtimestamp／UUID検証によりfilter構文へ任意文字列を混ぜない。結果のnull／不正型／DB error／権限照合失敗は空配列成功へ変換しない。必要最小の既存連絡先とreceipt／claim状態を返しno-storeを付ける。検索条件変更はcursorをリセット、古いfetch結果は世代／abortで破棄する。承認は公開完了と区別し、更新HTTP成功でもsuccess契約を確認する。

必須gateは未認証／非admin／認可DB失敗でservice queryなし、CSRF、壊れたJSON／cursor、100件超・同時刻・NULL日付・ページ間新規追加、完全一致filter、null結果／取得失敗、古い応答の逆転、検索／状態変更とcursorの整合、React表示と操作の実テスト。read-only検索で本番データを実際に取得しない。変更単位の独立レビューと最新CIを必須とする。既存審査・claimの本番書込み、自由な重複統合、owner方針変更はこのnodeに含めない。

設計レビューでemail_canonicalはGmailのdot／plus／domain統一まで含むと再確認したため、検索には使用しない。email列への保存値完全一致（大小文字も区別）を明記する。これは店舗同一性・所有権判定ではない。cursor時刻はDBの小数秒を丸めず保持し、NULL審査状態・旧状態も表示可能とする。DB／Authの詳細例外は固定カテゴリへ置換してから監視し、検索語・連絡先を通知しない。UIの検索変更は古い結果・loading・error・cursor・確認dialogを同時に無効化する。

独立実装レビューで、結果不明直後の再操作と審査競合をP2として確認。UIでは結果不明時のmutationを再照合まで止める。単なるstatus CASではAPI上の再審査でpending→rejected→pendingと戻るABAを防げないため、追加migration007でsalons.review_revision（非負integer・初期0）と全UPDATE時の単調増加triggerを導入する。既存行の業務内容は変えずversionを付け、旧状態と取得時revisionの両方が一致した更新だけを許可する。旧ブラウザなどrevision欠損は更新せず再読込を要求する。申込一覧にもrevisionを返し、APIの0行更新は存在しないか変更済みとして409にする。DB triggerがrevisionの手動書換え・同値更新・reopenでも必ず旧値+1とし、過去の状態への回帰で古い判断を受け入れない。上限overflowは失敗して進めない。本番007は事前計画・適用・履歴照合なしで実行しない。正常／競合／ABA／権限／通知への生エラー非露出を追加gateにする。

### 10.22．4cfeb82eの失敗原因とREG-03検証

4cfeb82e959b90397b451d7d5268d22a8643bf60のCI 36245276688はLint／型／Security成功、Unitは8344成功・1失敗（生成型とschema snapshotの不一致）、E2Eは287成功・10失敗。登録10失敗は標準fetchを依存オブジェクトのメソッドとして呼び、ブラウザが送信前にIllegal invocationを投げる共通原因だった。秘密情報なし・外部通信遮断の実Chromiumで同じ呼び出しを失敗、切り離した関数呼び出しを成功と確認した。coordinatorを修正し、receiverを検査する単体回帰を追加した。新SHAの実登録E2Eは未実行であり、本番解消とは扱わない。

schema CI 36245276718では完全schemaのfingerprintは一致したが、setup fixtureのsalons行に必須representative_name／contact_nameがなく失敗した。縮小schemaの合格が完全schemaの合格でないことを実際に検出したもので、fixtureへ合成値を追加しDB制約は維持する。007による次のfingerprint差分は完全schemaを適用したCI生成物と照合して更新する。生成済本番型を候補DDLに合わせて書き換え、未適用を隠すことはしない。

REG-03は7suite189test成功。検索・入力契約・一覧API・審査API・browser coordinatorの対象5moduleは分岐／関数／行100％。別途5suite47のroute／migration／隔離E2E guardが成功した。これらは秘密情報なし・network遮断のlocal結果で、全体CIの代替ではない。private Unix socketのみの縮小PostgreSQLへ実007と審査SQL fixtureを適用し、CAS、ABA、revision強制加算、overflow拒否を確認後停止した。125行・同時刻microseconds・NULL日付の3ページ、認可、競合、reopen後ABA、実画面検索を検証するdisposable Supabase E2Eを追加したが、実行成功は次CIで確認する。

独立レビューでE2E施設fixtureの必須所在地不足を修正し、検索markerとfixture施設名も分離した。限定再レビューの追加P0〜P3は0件（E2E SHA256 e1eceadd79192e4bd270f757358e95b194ab95c382ec4e290efc2d5dd5c1fa64）。006/007の列を期待schemaとlimit0 Contractに明示する。全体台帳、複数店舗方針、本番schema／履歴、SMTP／OAuth、実申込調査とrelease gateは引き続き未完了。

最新差分の全体tscと対象ESLintも成功。審査APIのrevision絞り込みだけを一時除去すると46test中3件が失敗し、直ちに同じ絞り込みを復元すると46件成功した。復元後sourceも直接照合した。この直列検証中には他の編集・stage・pushを行っていない。

007の完全schema fingerprintはCI36247401657／artifact10907444497から取得し、旧期待値に対して列・CHECK・関数権限・invoker関数・triggerの5行追加だけと親と独立レビューで確認した。artifactとのbytes一致を確認した期待値のSHA256は2a03fa6108eac8ab9cf750b1ef6a737f717270ee9b7ddb0a4c6358f94dcae310。先行runではfingerprint失敗により後続fixtureは未実行である。

### 10.23．REG-11設定保存の成功判定修正準備

settingsの通常保存・公開状態更新はどちらもservice更新のerrorしか見ず、認可照会後に施設が存在しなくなった場合など0行更新をokへ変換する。親が現在routeの両経路を直接確認した。対象はこの2経路と回帰テスト。更新行IDをDB応答で照合し、0行／欠損／別IDなら409・成功監査なし、DB失敗は500とする。外側の例外は固定カテゴリの監視と500へ接続し、エラーの生値・所在地等を監視へ送らない。非対象のowner方針・課金・本番データ操作は行わない。membershipの同時失効に対する完全なtransaction保証はこの0行対策だけでは主張しない。正常／異常／status3種／CSRF／認可失敗／0行を必須とし、独立レビュー後に反映する。

REG-12の調査ではcitySlugsが部分masterであり、areasには市と行政区の両粒度があることを再確認した。単純な市への切詰めは区検索を壊すため採用しない。canonical v1の変更は旧intent replayを壊す可能性があり、互換性設計と検索粒度の統一が残る。未収録自治体を不正として拒否する方針は採用しない。

REG-11局所修正は49test、対象routeのstatement／branch／function／line全100％、全体tsc、対象lintが成功。通常更新のID照合を一時的に無効化したfocused runは4件が200誤成功として失敗（残45件は当該mutation実験だけ対象外）、直ちに復元したfull runでは49件全成功。復元後route hashは独立レビュー時と同じ060d1d4a6235d184cadafd79cce200b1403c844084cb13de97adbd94955a8f6d、破壊値の残存なし。独立レビューでは両更新経路・UI呼出元・DBガードを照合し追加P0〜P3なし。最新CI・本番証拠は未取得。

### 10.24．CL-AUD-99プロフィール更新の準備

認証済みユーザーのprofiles行が欠損してもPUTがerrorなしの0行更新をsuccessへ変換することを現在sourceで再確認した。未完了のアカウント削除との競合があり得るため、今回勝手にprofileを再作成しない。対象ユーザーIDの更新行を確認し、欠損は409と復旧案内へ分離する。生年月日の空文字はschema上有効なのにDATE列へ空文字を書こうとするためnull化を実装する。UI正常／失敗応答とdirty保持、認証・CSRF・IDスコープ、0行・DB例外・空日付の回帰を検証する。profile欠損の元原因や削除saga全体がこの変更だけで解決したとは扱わず、上流台帳を残す。実ユーザーや本番への書込みは行わない。

実装後API42件・実Reactフォーム10件が成功。欠損／取得失敗は空フォームを表示せず、再試行中もloading表示を維持する。HTTP成功だけでなくsuccess=trueを確認してdirty解除する。独立レビューで送信中の追加入力がresetで消えるP2を検出し、fieldsetで6入力を送信中無効化、遅延応答後の再編集を含む回帰で確認した。ID確認ガードを一時無効化すると4件が誤成功200として失敗、直ちに復元すると42件成功、復元hash e026ae4c9b93a8d857cad6cfcec0091eeb70954c8de595735e6d131b1c6b9cf6一致。今回のmutation中は他の書込み・stage・pushなし。独立限定再レビューの追加P0〜P3は0件。LINE解除・アカウント削除全体の完了を意味しない。

### 10.25．9a99e0f0のCI再判定とSSR準備前入力

CI36247401655はUnit8416成功・1失敗（schema snapshotと本番由来型の不一致）、Lint／型／Security成功、E2E298成功・1失敗。以前の登録10件の失敗は再現しなくなった。E2E残存はMobile Safariの申込一覧で125行検索APIは50／50／25を成功した一方、画面検索の最初の50件期待が18／16／11件だった。SSRに有効な入力があるためhydration前入力をReact stateが受け取れず全件検索になる経路が存在する。初期HTMLの検索fieldset／再読込を無効化しeffect開始後に有効にする修正とSSR回帰を追加。実E2Eは検索POST本文も確認し、単なる待ち時間延長で隠さない。今回修正でのWebKit成功は次CI待ちである。

プロフィールの空日付を実DBのNULLへ保存し、実画面編集、直後にそのテストが新規作成した本人のprofilesのみ欠損fixtureへ変えてPUT409／復旧UIを確認するdisposable CI E2Eを追加した。本番ユーザー・送信・auth.users削除は含めず、固定HTTPS CI guardとtrace非保存を維持する。独立レビューがFK／trigger／対象スコープを照合した。今回の4suite70test、全体tscは成功。ESLintはerror0、既存のアカウント削除後window.location移動warning1でありwarningゼロとは報告しない。hosted Contractは3失敗・17skip・14成功で、skipを成功に数えず本番schema／型再生成／接続gateは未完了である。

### 10.26．地域検索の境界・障害表示の準備

REG-12と検索台帳の直接経路を確認した。現在city検索は県を使わず、region／stationは空filterで全国検索になる。areas DB errorはnull／emptyへ変わり、area pageはfacility query errorも無視して掲載なし表示する。対象はareas helperとエリアページ、公開card viewの限定検索、関連テスト。既存の汎用／GPS検索、住所writerのcanonical v1、店舗所有方針、DB実データは変更しない。

親階層はID単位に読んで、一覧取得上限による祖先欠落を回避する。欠損、循環、過深、取得errorは不完全なpathや404へ変換しない。県、市＋県、駅＋祖先県／市、地方の子県集合という明示scopeを公開検索へ渡し、不明なscopeから全国検索へfallbackしない。駅は説明文でなく最寄駅専用列を検索し、LIKEの特殊文字をescapeする。子県がない地方は下位エリア準備中と表示する。公開viewとpublished条件、安定したrating＋id順、countとpageを維持する。DB障害は既存error boundaryの再試行へ接続する。正常4type、同名別県、欠損／cycle／depth、DB失敗、空集合、駅特殊文字、page境界とUI異常表示を必須gateとし、独立反証後に実装を検証する。

地域検索実装のローカル検証：祖先をID単位で取得し、exact countとの差で上限切れを拒否する。県／市／駅／地方を明示scopeへ変換し、未完成階層を広域検索にfallbackさせない。施設は公開view・published限定、rating＋id順で取得する。駅LIKE文字をescapeし、DB障害は空件数に変換せずerror boundaryへ渡す。focused Jest 4suite・52test成功、areas／area-facilitiesのbranches・functions・lines・statementsは100％、対象ESLint、全体tsc、schema snapshot生成確認も成功。変更は未pushで、E2E、最新CI、独立レビュー、本番データの階層実態は未確認。CIにあるProfile E2Eのログイン経路修正も、この変更と同じ未push差分である。

### 10.27．Profile E2Eの認証状態分離と本番migration照合
