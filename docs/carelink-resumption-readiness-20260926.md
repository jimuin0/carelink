...
実DBからの型生成artifactをCIへ追加した。新table／RPCの生成型とfingerprint期待値は、次CIの実DB生成結果を照合して反映する。今回のmigration・SQL fixture・Storage・競合の実行はまだ未完了。本番反映・新受付経路の有効化・merge・deployは行っていない。
