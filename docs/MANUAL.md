...
| Stripe | ✅ 設定済み（テストモード） | STRIPE_SECRET_KEY/STRIPE_WEBHOOK_SECRET設定済み（2026-04-07、sk_test_）。Webhook送信先「CareLink本番Webhook」`/api/payment/webhook`、7イベント受信（checkout.session.completed / customer.subscription.created・updated・deleted / invoice.payment_succeeded・failed / payment_intent.payment_failed）。本番化は審査後 |
...
| 優先度 | タスク | 内容 |
|:---:|------|------|
| 高 | Stripe 本番化 | テストモード（sk_test_）→ 本番モード（sk_live_）切替。審査通過後にVercelの`STRIPE_SECRET_KEY`/`STRIPE_WEBHOOK_SECRET`をlive用に入れ替え、本番Webhookエンドポイント再登録（7イベント） |
...
| 中 | Resend ドメイン認証 | 送信元を `onboarding@resend.dev` → `noreply@carelink-jp.com` に切替（SPF/DKIM設定） |
...
## Vercel CRON_SECRET注意事項

`CRON_SECRET` 環境変数に末尾の空白文字（whitespace）が含まれるとビルドが失敗する。Vercel環境変数設定時は値の前後に余分なスペースがないことを確認すること。

## OG Image

- v8.x で `/api/og` 動的生成（`@vercel/og`）は削除済み。`@vercel/og`パッケージも未導入。
- 現在は `public/` の静的OG画像（`/og.png` 等）を施設詳細・トップで利用。
