...
| 19 | `/api/line/webhook` | POST | - | - | LINE Messaging Webhook（署名検証・フォロー応答・メッセージ応答） |
| 20 | `/api/cron/daily-summary` | GET | CRON | - | 日次売上集計（前日分をdaily_revenue_summaryに集計） |
| 21 | `/api/cron/customer-segment` | GET | CRON | - | 週次顧客RFM分析（customer_segmentsを更新） |
| 22 | `/api/admin/report` | GET | 必須 | - | 売上レポートCSVエクスポート（期間指定・権限チェック） |
| 23 | `/api/facility/setup` | POST | 必須 | 5回/60秒 | 施設自動作成+facility_membersにowner登録（セルフオンボーディング） |
| 24 | `/api/cron/review-request` | GET | CRON | - | 来店後レビュー依頼（完了24h後にメール+LINE送信） |
| 25 | `/api/payment/checkout` | POST | 必須 | 5回/60秒 | Stripe Checkout Session作成（予約決済） |
| 26 | `/api/payment/webhook` | POST | - | - | Stripe Webhook（payment_status更新+返金処理） |
| 27 | `/api/account/delete` | POST | 必須 | 5回/60秒 | アカウント+全データ削除（個人情報保護法対応） |
| 28 | `/api/referral` | GET/POST | 必須 | 5回/60秒 | 紹介コード取得/使用（双方ポイント付与） |
