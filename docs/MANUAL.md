...
│   │       ├── cron/                    … Vercel Cron 4本（booking-reminder/daily-summary/customer-segment/review-request）
...
### 6.1.1 マイグレーションファイル一覧（27ファイル、`combined_phase2_to_6.sql`を含む）
...
### 10.0 全APIルート一覧（31エンドポイント）

**2026-04-08追加（4個）**:
- `GET /api/health` — 外形監視用ヘルスチェック（DB疎通+応答時間+commit hash返却、200/503）
- `GET /api/sentry-check` — Sentry動作確認（`?fire=1&token=...`でテストエラー発火）
- `POST/GET /api/admin/jobs` — 求人管理API（POST=作成、GET=一覧）
- `GET/PUT/DELETE /api/admin/jobs/[id]` — 求人詳細API（個別取得・更新・削除）
