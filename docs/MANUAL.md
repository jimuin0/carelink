...
| `facility_jobs` | 施設別求人情報（id/facility_id FK/title/job_type/employment_type/salary_min/max/description/requirements/benefits/working_hours/holiday/is_active、v8.11、`/jobs` 一覧+`/jobs/[id]` 詳細+admin/jobs CRUD で利用） | 求人 |
...
| `20260407_seed_flag_and_jobs.sql` | `facility_profiles.is_seed BOOLEAN DEFAULT false` 追加（`scripts/cleanup-seed.mjs` で一括削除する用フラグ、部分インデックス付き） + `facility_jobs` テーブル新設（id/facility_id FK/title/job_type/employment_type/salary_min/max/description/requirements/benefits/working_hours/holiday/is_active/created_at、求人管理用、RLS有効） |
...
### 10.0 全APIルート一覧（32エンドポイント）

**v8.11 で表に新規追加された5個**（2026-04-07/08）:
...
- `GET /api/og` — 動的OG画像生成（@vercel/og、`?title=...&subtitle=...`、facility/jobs/blog/feature/[prefectureSlug] 等で利用）。**v8.10時点で「削除済み」と誤記載されていたが実装存在を確認しv8.11で復活**
