...
| `GOOGLE_RECAPTCHA_SECRET_KEY` | reCAPTCHA v3 秘密鍵（口コミBotスパム対策、v8.35追加） | - | サーバーのみ | Vercel |
...
> **補足**: 上記48エンドポイント中、`/api/og` はroute.tsx（`@vercel/og`使用）。CronエンドポイントはCRON_SECRET Bearer認証必須。
...
| 44(v8.43.0) | 50+件 | **v8.28〜v8.43 全実装のMANUAL統合更新**: §1ヘッダーv8.43.0に更新。§4.1 LINE Works5変数+GCal2変数+reCAPTCHA2変数追加（計10変数追加）。§4.3 .env.exampleにLINE Works/GCal/reCAPTCHA追加。§5.2.1 Vercel Cron 5→6本（newsletter-digest追加）。§6.1 Phase13(11テーブル+ALTER3件)+Phase14(8テーブル)追加。§6.1.1 migration注記追加。§8.2 admin pages 11件追加（cron-monitor/moderation/newsletters/community/manual/tutorials/job-applications/featured-ads/white-label + intake/[slug]）。§10.0 エンドポイント35→48本・新規13本テーブル追加（backup/newsletter/community/featured-ads/job-applications/white-label/google-calendar各種）。変更履歴v8.28〜v8.43の16エントリ追加。 |
