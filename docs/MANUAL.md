...
| `20260417_phase7_hpb_extensions.sql` | Phase 7 HPB超え拡張: `feature_articles`（特集記事）・`facility_qa`（Q&A/ステータス/is_public）・`chat_rooms`（UNIQUE facility_id+user_id）・`chat_messages`（Realtime対応）・`user_preferred_staff`（UNIQUE user_id+staff_id）・`review_helpful`（複合PK review_id+user_id）の6テーブル新設 + `facility_reviews` に `is_verified_visit BOOLEAN`/`photo_urls TEXT[]` 追加。全テーブルRLS有効・インデックス設定（v8.13） |
