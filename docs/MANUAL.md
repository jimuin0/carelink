...
| @anthropic-ai/sdk | AIチャット・レビュー要約・症状提案（Claude Haiku使用） | 0.90.0 |
| qrcode / @types/qrcode | QRコード生成（admin/qrcode） | 1.5.4 |
| @playwright/test | E2Eテスト（homepage/search/booking/auth） | 1.49.0 |
...
| `LINE_WORKS_CLIENT_ID` | LINE Works OAuth2 クライアントID（スタッフ通知Bot、v8.41追加） | - | サーバーのみ | Vercel |
| `LINE_WORKS_CLIENT_SECRET` | LINE Works OAuth2 クライアントシークレット（v8.41追加） | - | サーバーのみ | Vercel |
| `LINE_WORKS_SERVICE_ACCOUNT` | LINE Works サービスアカウントID（JWT Bearer Flow、v8.41追加） | - | サーバーのみ | Vercel |
| `LINE_WORKS_PRIVATE_KEY` | LINE Works RSA秘密鍵（PEM形式、JWT署名用Base64URLエンコード、v8.41追加） | - | サーバーのみ | Vercel |
| `LINE_WORKS_BOT_ID` | LINE Works Bot ID（v8.41追加） | - | サーバーのみ | Vercel |
| `GOOGLE_CLIENT_ID` | Google OAuth2 クライアントID（Googleカレンダー双方向同期、v8.42追加） | - | サーバーのみ | Vercel |
| `GOOGLE_CLIENT_SECRET` | Google OAuth2 クライアントシークレット（v8.42追加） | - | サーバーのみ | Vercel |
...
# LINE Works連携（スタッフ通知Bot、v8.41追加）
# LINE_WORKS_CLIENT_ID=your_line_works_client_id
# LINE_WORKS_CLIENT_SECRET=your_line_works_client_secret
# LINE_WORKS_SERVICE_ACCOUNT=your_service_account_id
# LINE_WORKS_PRIVATE_KEY=-----BEGIN RSA PRIVATE KEY-----\n...
# LINE_WORKS_BOT_ID=your_bot_id

# Google OAuth2（Googleカレンダー双方向同期、v8.42追加）
# GOOGLE_CLIENT_ID=your_google_client_id
# GOOGLE_CLIENT_SECRET=your_google_client_secret

# reCAPTCHA v3（Botスパム対策、v8.35追加）
# NEXT_PUBLIC_RECAPTCHA_SITE_KEY=your_recaptcha_site_key
# RECAPTCHA_SECRET_KEY=your_recaptcha_secret_key

# Anthropic AI（チャット/レビュー分析/症状提案、Claude Haiku使用）
...
| お気に入りダイジェスト | `/api/cron/favorites-digest` | `0 15 * * 0` | 毎週日曜 0:00 JST（UTC日曜15:00）。お気に入り施設の新着クーポン・ブログを通知 |
| キャンセル待ち通知 | `/api/cron/waitlist-notify` | `0 * * * *` | 毎時 00分。キャンセル発生→waitlistユーザーにメール通知 |
| Webhookリトライ | `/api/cron/webhook-retry` | `*/15 * * * *` | 15分ごと。webhook_retry_queueの失敗Webhookを指数バックオフ3回で再試行 |
...
**Phase 7: HPB超え拡張（6テーブル + カラム追加）**（`booking_menus`は構想のみで未実装）
...
**Phase 13: スケーリング・監視・セキュリティ（v8.28〜v8.40）**

| テーブル/変更 | 用途 | サイト |
|-------------|------|--------|
| `cron_logs` | Cronジョブ実行ログ（job_name/status/duration_ms/error_message、v8.32） | 管理 |
| `audit_logs` | 監査ログ（actor_id/action/table_name/record_id/old_data/new_data、90日TTL自動削除、v8.33） | 管理 |
| `booking_waitlist` | キャンセル待ち（facility_id/staff_id/menu_id/desired_date/notify_before_hours、v8.34） | 検索・管理 |
| `intake_form_templates` | 施術前デジタル問診票（template_name/fields JSONB/is_active、v8.36） | 管理 |
| `intake_submissions` | 問診票回答（booking_id/template_id/responses JSONB、v8.36） | 管理 |
| `moderation_queue` | コンテンツモデレーション（content_type/content_id/reason/status/priority、v8.37） | 管理 |
| `webhook_retry_queue` | Webhook失敗リトライ（url/payload/attempts/next_retry_at/status、指数バックオフ3回、v8.35） | システム |
| `facility_badges` | 施設認証バッジ（badge_type: phone/identity/site_visit、verified_at、v8.32） | 検索 |
| `facility_card_view` | マテリアライズドビュー（CONCURRENT REFRESH対応、集計クエリ高速化、v8.29） | 検索 |
| ALTER `facility_profiles` | PostGIS `location GEOGRAPHY(POINT, 4326)` 追加（空間インデックス付き、ST_DWithin RPC、v8.42） | 検索 |
| ALTER `staff_profiles` | `line_works_channel_id TEXT` 追加（スタッフ別LINE Works通知先、v8.41） | 管理 |

**Phase 14: ニュースレター・コミュニティ・広告（v8.41〜v8.42）**

| テーブル | 用途 | サイト |
|---------|------|--------|
| `newsletter_campaigns` | ニュースレターキャンペーン（subject/body/status/sent_count/facility_id、v8.42） | 管理 |
| `newsletter_subscriptions` | ニュースレター購読（email/user_id/facility_id/is_active、v8.42） | マイページ |
| `community_posts` | コミュニティ投稿（title/body/category/author_id/status/views、v8.42） | 管理・将来 |
| `community_replies` | コミュニティ返信（post_id/author_id/body、v8.42） | 管理 |
| `community_likes` | コミュニティいいね（post_id/user_id UNIQUE、v8.42） | 管理 |
| `featured_slots` | 広告枠（facility_id/slot_type/start_at/end_at/price/status/stripe_payment_intent_id、v8.43） | 管理・検索 |
| `booking_calendar_events` | Googleカレンダー同期（booking_id/user_id/google_event_id、v8.42） | マイページ |
| `support_tickets` | お問い合わせチケット（subject/body/status/priority/internal_notes、v8.38） | 管理 |

**Phase 15: 収益化・医療・AI・インフラ拡張（v8.41〜v8.43）**

| テーブル | 用途 | サイト |
|---------|------|--------|
| `subscription_plans` | サブスク料金プラン（plan_name/price/interval/features JSONB） | 管理 |
| `user_subscriptions` | ユーザーサブスク購読状態（user_id/facility_id/plan_id/status/stripe_sub_id） | マイページ・管理 |
| `service_packages` | サービスパッケージ（名前/回数/有効期限/price） | 管理 |
| `user_packages` | ユーザーパッケージ購入（user_id/package_id/remaining_count） | マイページ・管理 |
| `group_bookings` | グループ予約（organizer_id/facility_id/max_participants/join_code） | 検索・管理 |
| `group_booking_participants` | グループ予約参加者（group_booking_id/user_id） | 検索 |
| `telehealth_sessions` | 遠隔診療セッション（booking_id/provider/room_url/status） | マイページ・管理 |
| `treatment_records` | 治療履歴（patient_id/facility_id/treatment_date/notes/staff_id） | 管理 |
| `treatment_plans` | 治療計画（patient_id/title/sessions/start_date/end_date） | 管理 |
| `nps_surveys` | NPS調査（user_id/booking_id/score/comment） | システム |
| `ab_tests` | A/Bテスト（name/variant_a/variant_b/start_at/end_at/metric） | 管理 |
| `feature_flags` | フィーチャーフラグ（flag_name/is_enabled/rollout_percentage） | 管理 |
| `api_keys` | 外部API公開用キー（key_hash/facility_id/scopes/last_used_at） | 管理 |
| `white_label_domains` | ホワイトラベルドメイン（domain/facility_id/dns_txt_token/verified_at） | 管理 |
| `platform_blog_posts` | プラットフォームコラム（施設ブログとは別、CareLink運営コンテンツ） | 管理・公開 |
| `email_unsubscribes` | メール配信停止（email/category/unsubscribed_at、CAN-SPAM対応） | システム |
| `area_seo_contents` | エリアSEOコンテンツ（prefecture/city/content/highlights、v8.11） | 検索 |
| `stripe_payments` | Stripe支払い詳細（payment_intent_id/amount/status/metadata JSONB） | 管理 |
| `review_flags` | 口コミスパムフラグ（review_id/reason/ip_address/auto_flagged） | 管理 |
...
### 6.1.1 マイグレーションファイル一覧（30ファイル+、`combined_phase2_to_6.sql`を含む）

> Phase 13+の機能（v8.28〜）はコード内で`createServiceRoleClient()`経由のマイグレーションまたはSupabase Dashboardで適用。以下は既知のファイルのみ記載。

| ファイル | 内容 |
|---------|------|
| `20260320000001_initial_tables.sql` | 初期テーブル: salons, job_seekers, contacts |
| `20260321000004_facilities_phase1.sql` | Phase 1: facility_profiles（施設公開データ） |
| `20260321000001_enable_rls.sql` | RLS有効化: salons, job_seekers, contacts（INSERT only） |
| `20260321000002_storage_policy.sql` | Storageポリシー: carelink-uploads（匿名アップロード+公開読取） |
| `20260321000003_contacts_phone.sql` | contacts.phone カラム追加 |
| `20260322000001_reviews_inquiries.sql` | facility_reviews, facility_inquiries + インデックス |
| `20260323000001_phase2_users_search.sql` | Phase 2: profiles, favorites, areas, view_count |
| `20260323000002_phase3_staff_coupons.sql` | Phase 3: staff_profiles, coupons, coupon_menus, menu_staff |
| `20260323000003_phase4_bookings.sql` | Phase 4: staff_schedules, schedule_overrides, bookings |
| `20260323000004_phase5_admin.sql` | Phase 5: facility_members, customer_visits |
| `20260323000005_phase6_advanced.sql` | Phase 6: treatment_catalogs, blog_posts, review_replies, user_points |
| `20260326000001_salons_extend.sql` | salons拡張: contact_phone, website, nearest_station, features[] |
| `20260328000001_performance_indexes.sql` | パフォーマンスインデックス4件（実行済み） |
| `20260328000002_reviews_extend.sql` | facility_reviews拡張: 5軸評価, photo_urls, is_verified_visit |
| `20260330000001_phase_c_infra.sql` | push_subscriptions, facility_card_view（VIEW、検索カード用集約）, 追加インデックス |
| `20260331000001_data_enrichment.sql` | シードデータ: スタッフ経験年数, スケジュール, ブログ, カタログ, エリア階層 |
| `20260331000002_push_subscriptions_and_indexes.sql` | push_subscriptions再構築（制約+RLS） |
| `combined_phase2_to_6.sql` | Phase 2-6統合マイグレーション（冪等トランザクション） |
| `20260404000003_line_integration.sql` | Phase 8: LINE連携（line_user_links/facility_line_settings/line_notification_logs + RLS + インデックス） |
| `20260404000002_dashboard_enhancement.sql` | Phase 9: ダッシュボード強化（daily_revenue_summary/customer_segments/facility_notification_settings + RLS + インデックス） |
| `20260404000001_acupuncture_specialization.sql` | Phase 10: 鍼灸院特化（symptoms 30件シード/facility_symptoms/facility_certifications + facility_menus ALTER×3 + staff_profiles ALTER×1） |
| `20260405000001_booking_conflict_constraint.sql` | 予約競合防止RPC（create_booking_atomic、FOR UPDATEロック） |
| `20260405000003_stripe_and_policies.sql` | Stripe決済（bookingsにpayment_status/stripe_payment_intent_id/paid_amount追加）+ facility_cancel_policies |
| `20260405000002_referral_program.sql` | 紹介プログラム（referral_codes/referral_uses） |
| `20260406000001_rls_authenticated_fixes.sql` | RLS修正: authenticated SELECT権限追加（facility_profiles/menus/photos/reviews/inquiries）+ bookings INSERT権限 + bookings.points_usedカラム追加 |
| `20260407000002_stripe_events.sql` | Stripe Webhook冪等性用 `stripe_events` テーブル新設（id PK/type/processed_at、service_role専用RLS）+ `bookings.payment_status` CHECK制約に `'failed'` 追加 |
| `20260407000001_seed_flag_and_jobs.sql` | `facility_profiles.is_seed BOOLEAN DEFAULT false` 追加（`scripts/cleanup-seed.mjs` で一括削除する用フラグ、部分インデックス付き） + `facility_jobs` テーブル新設（id/facility_id FK/title/job_type/employment_type/salary_min INT/salary_max INT/salary_note/description/requirements/benefits/is_seed/created_at/updated_at、求人管理用、RLS有効） |
| `20260417000015_gbp_integration.sql` | Phase 12: `gbp_posts` テーブル新設（GBP投稿管理）+ `gbp_audit_cache` テーブル新設（診断スコアキャッシュ）+ `facility_profiles` に `gbp_place_id/gbp_cid/gbp_connected_at` 追加（v8.12） |
| `20260417000017_google_rating_columns.sql` | `facility_profiles` に `google_rating NUMERIC(2,1)/google_review_count INTEGER` 追加 + `facility_card_view` 再定義（google_rating/google_review_count を含む）+ `GRANT SELECT TO anon, authenticated`（v8.12） |
| `20260417000024_phase7_hpb_extensions.sql` | Phase 7 HPB超え拡張: `feature_articles`（特集記事）・`facility_qa`（Q&A/ステータス/is_public）・`chat_rooms`（UNIQUE facility_id+user_id）・`chat_messages`（Realtime対応）・`user_preferred_staff`（UNIQUE user_id+staff_id）・`review_helpful`（複合PK review_id+user_id）の6テーブル新設 + `facility_reviews` に `is_verified_visit BOOLEAN`/`photo_urls TEXT[]` 追加。全テーブルRLS有効・インデックス設定（v8.13） |
...
CREATE TABLE salons (
  ...
  -- 拡張カラム（20260326000001_salons_extend.sql）
...
| `/admin/cron-monitor` | `admin/cron-monitor/page.tsx` | Dynamic | Cronジョブ実行監視（cron_logsテーブル表示、v8.32） |
| `/admin/moderation` | `admin/moderation/page.tsx` | Dynamic | コンテンツモデレーション（承認/却下/エスカレ、moderation_queue、v8.37） |
| `/admin/newsletters` | `admin/newsletters/page.tsx` | Dynamic | ニュースレター管理（キャンペーン作成・配信・購読者管理、v8.42） |
| `/admin/community` | `admin/community/page.tsx` | Dynamic | コミュニティ管理（投稿モデレート・v8.42） |
| `/admin/manual` | `admin/manual/page.tsx` | Dynamic | 操作マニュアル（印刷/PDF対応、v8.42） |
| `/admin/tutorials` | `admin/tutorials/page.tsx` | Dynamic | ビデオチュートリアル（YouTube埋め込み、v8.42） |
| `/admin/job-applications` | `admin/job-applications/page.tsx` | Dynamic | 求人応募管理（採用手数料管理、v8.42） |
| `/admin/featured-ads` | `admin/featured-ads/page.tsx` | Dynamic | 広告枠管理（Stripe決済連携・featured_slots、v8.43） |
| `/admin/white-label` | `admin/white-label/page.tsx` | Dynamic | ホワイトラベル管理（DNS TXT認証・カスタムドメイン、v8.41） |
| `/intake/[slug]` | `intake/[slug]/page.tsx` | Dynamic | 施術前デジタル問診票入力（v8.36） |
| `/admin/ab-tests` | `admin/ab-tests/page.tsx` | Dynamic | A/Bテスト管理（feature_flags/ab_tests、v8.41） |
| `/admin/accounting` | `admin/accounting/page.tsx` | Dynamic | 会計管理（freee/MoneyForward CSV エクスポート、v8.41） |
| `/admin/api-keys` | `admin/api-keys/page.tsx` | Dynamic | 外部API公開用キー管理（api_keys CRUD、v8.41） |
| `/admin/chain` | `admin/chain/page.tsx` | Dynamic | チェーン施設統合管理（複数施設レポート/一括設定、v8.41） |
| `/admin/email-setup` | `admin/email-setup/page.tsx` | Dynamic | メール設定（Resend送信者設定・テンプレート） |
| `/admin/feature-flags` | `admin/feature-flags/page.tsx` | Dynamic | フィーチャーフラグ管理（段階的ロールアウト、v8.41） |
| `/admin/funnel` | `admin/funnel/page.tsx` | Dynamic | ファネル分析（検索→詳細→予約のコンバージョン） |
| `/admin/group-bookings` | `admin/group-bookings/page.tsx` | Dynamic | グループ予約管理（参加者確認・承認、v8.42） |
| `/admin/jobs/applications` | `admin/jobs/applications/page.tsx` | Dynamic | 求人応募詳細一覧（job_applications、v8.42） |
| `/admin/line-richmenu` | `admin/line-richmenu/page.tsx` | Dynamic | LINE RichMenuテンプレート管理 |
| `/admin/medical-docs` | `admin/medical-docs/page.tsx` | Dynamic | 医療書類管理（保険請求書・紹介状テンプレート） |
| `/admin/medical-docs/insurance` | `admin/medical-docs/insurance/page.tsx` | Dynamic | 保険診療管理 |
| `/admin/medical-docs/referral` | `admin/medical-docs/referral/page.tsx` | Dynamic | 紹介状管理 |
| `/admin/packages` | `admin/packages/page.tsx` | Dynamic | サービスパッケージ管理（service_packages CRUD、v8.41） |
| `/admin/payments` | `admin/payments/page.tsx` | Dynamic | 支払い管理（stripe_payments一覧・レシート） |
| `/admin/platform-blog` | `admin/platform-blog/page.tsx` | Dynamic | プラットフォームコラム管理（施設ブログとは別、CareLink運営コンテンツ） |
| `/admin/platform-blog/new` | `admin/platform-blog/new/page.tsx` | Dynamic | コラム新規作成 |
| `/admin/platform-blog/[id]/edit` | `admin/platform-blog/[id]/edit/page.tsx` | Dynamic | コラム編集 |
| `/admin/qrcode` | `admin/qrcode/page.tsx` | Dynamic | QRコード生成・ポスター印刷用（qrcodeライブラリ） |
| `/admin/subscription-plans` | `admin/subscription-plans/page.tsx` | Dynamic | サブスク料金プラン管理（subscription_plans、v8.41） |
| `/admin/telehealth` | `admin/telehealth/page.tsx` | Dynamic | 遠隔診療セッション管理（telehealth_sessions、v8.41） |
| `/admin/treatment-plans` | `admin/treatment-plans/page.tsx` | Dynamic | 治療計画管理（treatment_plans、v8.41） |
| `/admin/treatment-records` | `admin/treatment-records/page.tsx` | Dynamic | 治療履歴記録（treatment_records、v8.41） |
| `/admin/uptime` | `admin/uptime/page.tsx` | Dynamic | サイト稼働時間監視ダッシュボード |
| `/admin/usage-stats` | `admin/usage-stats/page.tsx` | Dynamic | 利用統計（MAU/DAU、施設オーナー向け、v8.41） |

**マイページ（追加分）**

| パス | ファイル | レンダリング | 説明 |
|------|---------|:----------:|------|
| `/mypage/packages` | `mypage/packages/page.tsx` | Dynamic | 購入パッケージ一覧（残回数表示） |
| `/mypage/referral` | `mypage/referral/page.tsx` | Dynamic | 紹介コード・紹介実績・ポイント獲得状況 |
| `/mypage/settings` | `mypage/settings/page.tsx` | Dynamic | アカウント設定（通知・連携・削除） |
| `/mypage/subscriptions` | `mypage/subscriptions/page.tsx` | Dynamic | サブスク管理（プラン変更・解約） |

**LP追加ページ**

| パス | ファイル | レンダリング | 説明 |
|------|---------|:----------:|------|
| `/salon/cases` | `salon/cases/page.tsx` | Static | 導入事例ページ |
| `/salon/premium` | `salon/premium/page.tsx` | Static | プレミアム掲載プラン紹介 |
| `/symptoms` | `symptoms/page.tsx` | Static | 症状一覧ページ（/symptom/[slug]へのナビ） |
| `/unsubscribe` | `unsubscribe/page.tsx` | Dynamic | メールニュースレター購読解除 |
| `/group-booking/join/[code]` | `group-booking/join/[code]/page.tsx` | Dynamic | グループ予約参加（join_code入力） |
...
### 10.0 全APIルート一覧（86エンドポイント）

**v8.41〜v8.43 で新規追加（v8.13以降）**:
...
- `GET/PATCH/DELETE /api/admin/job-applications/[id]` — 応募詳細（v8.42）
- `GET/POST /api/admin/white-label` — ホワイトラベル設定（v8.41）
- `POST /api/admin/white-label/verify` — DNS TXT認証（v8.41）
- `GET /api/google-calendar` — Googleカレンダー連携状態確認（OAuth2、v8.42）
- `GET /api/google-calendar/callback` — OAuth2コールバック（v8.42）
- `POST /api/google-calendar/sync` — 予約→Googleカレンダー同期（v8.42）
...
| 43 | `/api/admin/job-applications/[id]` | GET/PATCH/DELETE | 必須 | 10回/60秒 | 応募詳細・ステータス更新・削除、v8.42 |
| 44 | `/api/admin/white-label` | GET/POST | 必須 | 5回/60秒 | ホワイトラベル設定（white_label_domains CRUD）、v8.41 |
| 45 | `/api/admin/white-label/verify` | POST | 必須 | 5回/60秒 | DNS TXT認証（カスタムドメイン確認）、v8.41 |
| 46 | `/api/google-calendar` | GET | 必須 | - | Googleカレンダー連携状態確認（OAuth2トークン有無）、v8.42 |
| 47 | `/api/google-calendar/callback` | GET | - | - | Google OAuth2コールバック（booking_calendar_eventsにトークン保存）、v8.42 |
| 48 | `/api/google-calendar/sync` | POST | 必須 | 5回/60秒 | 予約→Googleカレンダー同期（booking_idから予定作成/更新）、v8.42 |
| 49 | `/api/ab-test` | GET/POST | - | 10回/60秒 | A/Bテスト変数割り当て・結果記録（feature_flags/ab_tests） |
| 50 | `/api/admin/accounting-export` | GET | 必須 | - | 会計データCSVエクスポート（freee/MoneyForward形式） |
| 51 | `/api/admin/ai-support` | POST | 必須 | 5回/60秒 | AIサポートチャット（Anthropic Claude Haiku） |
| 52 | `/api/admin/api-keys` | GET/POST | 必須 | 5回/60秒 | APIキー管理（api_keys CRUD） |
| 53 | `/api/admin/api-keys/[id]` | GET/PUT/DELETE | 必須 | 5回/60秒 | APIキー詳細・更新・削除 |
| 54 | `/api/admin/facility-verify` | POST | 必須 | 5回/60秒 | 施設認証バッジ付与（facility_badges） |
| 55 | `/api/admin/packages` | GET/POST | 必須 | 5回/60秒 | サービスパッケージ管理 |
| 56 | `/api/admin/packages/[id]` | GET/PUT/DELETE | 必須 | 5回/60秒 | パッケージ詳細 |
| 57 | `/api/admin/review-summary` | GET | 必須 | 5回/60秒 | AIレビュー分析サマリー（Anthropic、Claude Haiku） |
| 58 | `/api/admin/subscription-plans` | GET/POST | 必須 | 5回/60秒 | サブスクプラン管理 |
| 59 | `/api/admin/subscription-plans/[id]` | GET/PUT/DELETE | 必須 | 5回/60秒 | プラン詳細 |
| 60 | `/api/admin/user-packages` | GET/POST | 必須 | 10回/60秒 | ユーザーパッケージ購入管理 |
| 61 | `/api/admin/user-subscriptions` | GET/POST | 必須 | 10回/60秒 | ユーザーサブスク管理 |
| 62 | `/api/booking/[id]/cancel-fee` | GET | 必須 | 10回/60秒 | キャンセル料金計算（facility_cancel_policies適用） |
| 63 | `/api/booking/[id]/ical` | GET | 必須 | 10回/60秒 | 予約iCal形式エクスポート（.icsダウンロード） |
| 64 | `/api/chat` | POST | 任意 | 5回/60秒 | AIチャットボット（Anthropic Claude Haiku、ユーザーサポート自動化） |
| 65 | `/api/cron/flag-reviews` | GET | CRON | - | スパム口コミ自動フラグ（毎時、同一IP/短時間大量投稿検知） |
| 66 | `/api/cron/webhook-retry` | GET | CRON | - | Webhook失敗リトライ（15分ごと、指数バックオフ3回） |
| 67 | `/api/cron/onboarding-followup` | GET | CRON | - | 登録3日後フォローメール（オンボーディング未完了施設） |
| 68 | `/api/cron/birthday-coupon` | GET | CRON | - | 誕生日クーポン自動配布（毎日） |
| 69 | `/api/cron/favorites-digest` | GET | CRON | - | お気に入りダイジェスト（毎週日曜、新着クーポン/ブログ通知） |
| 70 | `/api/cron/waitlist-notify` | GET | CRON | - | キャンセル待ち通知（毎時、waitlist→メール通知） |
| 71 | `/api/group-booking` | GET/POST | 任意 | 10回/60秒 | グループ予約作成・一覧取得 |
| 72 | `/api/group-booking/[id]` | GET/PATCH/DELETE | 必須 | 10回/60秒 | グループ予約詳細・更新・削除 |
| 73 | `/api/group-booking/join` | POST | 任意 | 10回/60秒 | グループ予約参加（join_code入力） |
| 74 | `/api/intake` | POST | 任意 | 5回/60秒 | 施術前デジタル問診票提出（intake_submissions INSERT） |
| 75 | `/api/nps` | POST | 必須 | 3回/60秒 | NPS調査回答提出（nps_surveys INSERT） |
| 76 | `/api/recommendations` | GET | 任意 | 20回/60秒 | AI施設推奨（閲覧/予約履歴ベースのパーソナライズ） |
| 77 | `/api/report` | GET | 必須 | 5回/60秒 | 売上詳細レポート（期間指定・集計） |
| 78 | `/api/review` | POST | 任意 | 3回/5分 | 口コミ投稿（reCAPTCHA v3検証付き） |
| 79 | `/api/stripe/checkout` | POST | 必須 | 5回/60秒 | Stripe Checkout Session作成（パッケージ/サブスク/広告枠） |
| 80 | `/api/stripe/receipt` | GET | 必須 | 10回/60秒 | 決済レシート取得（stripe_payments参照） |
| 81 | `/api/stripe/webhook` | POST | 署名検証 | - | Stripe Webhook統合（複数イベント処理） |
| 82 | `/api/symptoms/suggest` | GET | 任意 | 20回/60秒 | 症状AI提案（Anthropic、症状チェッカー用） |
| 83 | `/api/unsubscribe` | GET/POST | - | 5回/60秒 | ニュースレター購読解除（email_unsubscribes INSERT） |
| 84 | `/api/v1/bookings` | GET/POST | APIキー認証 | 20回/60秒 | 予約API v1（外部POS/会計ソフト連携） |
| 85 | `/api/v1/customers` | GET/POST | APIキー認証 | 20回/60秒 | 顧客API v1（外部連携） |
| 86 | `/api/waitlist` | GET/POST | 任意 | 10回/60秒 | キャンセル待ちリスト登録/確認（booking_waitlist） |

> **補足**: 上記86エンドポイント中、`/api/og` はroute.tsx（`@vercel/og`使用）。CronエンドポイントはCRON_SECRET Bearer認証必須。`/api/v1/*`はAPIキー（Bearerヘッダー）認証。
...
`supabase/migrations/20260328000001_performance_indexes.sql`（4件の部分インデックス、実行済み）:
...
| 36(v8.11.1) | 13件 | **MANUAL二次深調査監査**: §4.1環境変数表 `NEXT_PUBLIC_GSC_VERIFICATION_APEX`/`SENTRY_TEST_TOKEN` 追加（Critical 2件）。§4.3 .env.example コード例同期（Critical 1件）。§6.1.1 `20260407000001_seed_flag_and_jobs.sql` 追加（Critical 1件）。§6.1 Phase11テーブル一覧に `facility_jobs` + `facility_profiles.is_seed` ALTER 追加（Critical 2件）。§10.0 API数 31→32、`/api/og` 説明追加（Critical 1件 + Major 2件）。§3 ディレクトリ構成に `scripts/` (seed-facilities.mjs/cleanup-seed.mjs) と `LAUNCH-CHECKLIST.md` 追加（Minor 2件）。Critical 8件 / Major 3件 / Minor 2件 を全駆除 |
...
| 43(v8.13.0) | 7件 | **Phase 7 DB Migration実装**: `20260417000024_phase7_hpb_extensions.sql` 新規作成。`feature_articles`/`facility_qa`/`chat_rooms`/`chat_messages`/`user_preferred_staff`/`review_helpful` の6テーブル + `facility_reviews` に `is_verified_visit`/`photo_urls` 追加。全テーブルRLS+インデックス+GRANT設定。Supabase Realtime用 `chat_messages` publication追加。**MANUAL更新**: §6.1 Phase7「※要Migration実行」注記を削除（ファイル作成済み）、§6.1.1 migration数29→30、v8.13.0 changelog追記 |
| 44(v8.43.0) | 200+件 | **Exploreエージェント深調査によるMANUAL完全整合監査**: §1.4技術スタックに`@anthropic-ai/sdk 0.90.0`/`qrcode 1.5.4`/`@playwright/test 1.49.0`追加。§4.1 `RECAPTCHA_SECRET_KEY`誤記修正（`GOOGLE_RECAPTCHA_SECRET_KEY`→`RECAPTCHA_SECRET_KEY`）+`ANTHROPIC_API_KEY`追加+LINE Works/GCal各変数追加（計12変数追加）。§4.3 .env.exampleにANTHROPIC_API_KEY・reCAPTCHA変数名修正。§5.2.1 Vercel Cron 6→12本（onboarding-followup/birthday-coupon/flag-reviews/favorites-digest/waitlist-notify/webhook-retry 追加）。§6.1 Phase15（19テーブル: subscription_plans/user_subscriptions/service_packages/user_packages/group_bookings/telehealth_sessions/treatment_records/treatment_plans/nps_surveys/ab_tests/feature_flags/api_keys/white_label_domains/platform_blog_posts/email_unsubscribes/area_seo_contents/stripe_payments/review_flags+他）追加。§8.2 admin pages 26件追加+mypage 4件追加+LP 5件追加（ab-tests/accounting/api-keys/chain/email-setup/feature-flags/funnel/group-bookings/jobs/applications/line-richmenu/medical-docs各種/packages/payments/platform-blog各種/qrcode/subscription-plans/telehealth/treatment-plans/treatment-records/uptime/usage-stats）。§10.0 エンドポイント 48→86本、38本追加（ai-support/api-keys/packages/subscription-plans/group-booking/intake/nps/recommendations/review+reCAPTCHA/stripe/symptoms/unsubscribe/v1外部API/waitlist/cron×6本）。 |
...
| 日付 | バージョン | 内容 |
|------|-----------|------|
| 2026-04-17 | 8.44 | **MANUAL完全整合監査（Exploreエージェント深調査）**: 技術スタックに`@anthropic-ai/sdk`/`qrcode`/`@playwright/test`追加。`RECAPTCHA_SECRET_KEY`誤記修正。`ANTHROPIC_API_KEY`追加。Vercel Cron 6→12本（6本追加）。Phase15 DBテーブル19個追加。admin pages 26+4+5=35件追加。APIエンドポイント 48→86本（38本追加、v1外部API/AI系/グループ予約/サブスク/パッケージ/遠隔診療/NPS/A-Bテスト/webhook-retry等）。 |
| 2026-04-17 | 8.43 | **LINE Works通知統合 + 広告枠Stripe決済 + ニュースレター配信Cron**: (1) `src/lib/integrations/line-works.ts` 新規（JWT Bearer Flow + Bot API v2.0 + sendLineWorksMessage ヘルパー）。booking/cancel/change API に統合。`staff_profiles.line_works_channel_id` ALTER追加。スタッフ編集UIにLINE Works通知設定追加。(2) 広告枠購入に Stripe Checkout Session 統合（`/api/admin/featured-ads` POST→Stripe決済→`featured_slots.status='active'`）。(3) `newsletter-digest` Cronジョブを vercel.json に登録（毎月1日10:00 JST）。(4) Stripe API バージョン `2025-04-30.basil` に統一。(5) supabase-serviceインポートをsupabase-serverに統一。 |
| 2026-04-17 | 8.42 | **15+ロードマップアイテム実装（ニュースレター・PostGIS・GCal・広告・E2E・コミュニティ）**: newsletter_campaigns/newsletter_subscriptions + admin/newsletters UI + /api/admin/newsletter。PostGIS移行（ST_DWithin RPC + geography column + 空間インデックス）。Googleカレンダー双方向同期（OAuth2 + booking_calendar_events + /api/google-calendar/sync）。featured_slots + admin/featured-ads UI。Playwright E2Eテスト（homepage/search/booking/auth/facility）。community_posts/replies/likes + admin/community UI。admin/manual（印刷/PDF対応）。admin/tutorials（YouTube埋め込み）。admin/job-applications UI。admin/white-label UI + DNS TXT認証 API。 |
| 2026-04-17 | 8.41 | **20+ロードマップアイテム実装（Stripe拡張・i18n・グループ予約・AIチャットボット等）**: Stripe サブスクリプション/回数券/月額プラン。多言語対応（英語/中国語/韓国語）。グループ予約（複数人同時予約）。AIチャットボット（ユーザーサポート自動化）。パーソナライズ推薦（閲覧/予約履歴ベース）。テレヘルス/オンライン相談。A/Bテスト基盤・NPS調査。施術履歴トラッキング・治療計画。保険請求書/紹介状テンプレート。チェーン店一括管理・ホワイトラベル（white_label_domains + DNS TXT認証）。外部API公開（POS/会計ソフト連携）。施設オーナー利用状況分析（MAU/DAU）。 |
| 2026-04-17 | 8.40 | **Redisキャッシュ層・ユニットテスト・CSP・コミュニティ基盤**: `src/lib/redis.ts` 新規（cacheGet/Set/Del/cachedFetch/queue/sessionのAPI + getPopularFacilities 10分キャッシュ）。redis.test.ts + booking-validation.test.ts 追加。CSP `strict-dynamic` progressive enhancement追加。`.github/workflows/dependency-update.yml`（毎週月曜 npm audit）。`middleware DB問い合わせキャッシュ`（cookie-based 5min TTL）。 |
| 2026-04-17 | 8.39 | **DBバックアップ管理API・ROADMAP更新**: `/api/admin/backup` 新設（GET: Supabase Pro自動日次バックアップ確認+重要テーブル行数チェック、POST: CSVエクスポート）。TOP-ROADMAP 8項目追加済みに更新。 |
| 2026-04-17 | 8.38 | **問い合わせチケット管理**: `support_tickets` テーブル（status/priority/internal_notes/email_replied）+ admin/support UI（ステータス管理・優先度・内部メモ・メール返信） |
| 2026-04-17 | 8.37 | **コンテンツモデレーション**: `moderation_queue` テーブル（content_type/content_id/reason/status/priority）+ admin/moderation UI（承認/却下/エスカレーション）。不正レビュー自動フラグ（同一IP/短時間大量投稿検知）。通報機能。 |
| 2026-04-17 | 8.36 | **施術前デジタル問診票**: `intake_form_templates`（template_name/fields JSONB/is_active）+ `intake_submissions`（booking_id/template_id/responses JSONB）テーブル。`/intake/[slug]` ページ（IntakeFormコンポーネント）。 |
| 2026-04-17 | 8.35 | **reCAPTCHA v3 + Webhook失敗リトライ**: `src/lib/recaptcha.ts` 新規（口コミ投稿API統合、Botスパム対策）。`webhook_retry_queue` テーブル（url/payload/attempts/next_retry_at/status）+ 15分Cron + 指数バックオフ3回。`NEXT_PUBLIC_RECAPTCHA_SITE_KEY` / `GOOGLE_RECAPTCHA_SECRET_KEY` 環境変数追加。 |
| 2026-04-17 | 8.34 | **キャンセル待ち機能**: `booking_waitlist` テーブル（facility_id/staff_id/menu_id/desired_date/notify_before_hours）。WaitlistFormコンポーネント（施設詳細ページ統合）。キャンセル発生→1時間以内にCron通知。 |
| 2026-04-17 | 8.33 | **監査ログ**: `audit_logs` テーブル（actor_id/action/table_name/record_id/old_data JSONB/new_data JSONB/created_at、90日TTL自動削除）。`src/lib/audit-logger.ts`（logAuditEvent ヘルパー）。 |
| 2026-04-17 | 8.32 | **施設認証バッジ + Cron実行監視**: `facility_badges` テーブル（badge_type: phone/identity/site_visit、verified_at）。FacilityHeaderに緑バッジ表示。`cron_logs` テーブル（job_name/status/duration_ms/error_message）。admin/cron-monitor UI。Sentry tracesSampleRate=0.02でクライアント有効化。 |
| 2026-04-17 | 8.29 | **facility_card_view マテリアライズドビュー化**: `facility_card_view` を通常VIEWからMATERIALIZED VIEWに変換。`REFRESH MATERIALIZED VIEW CONCURRENTLY` 対応（読み取りブロックなし）。INSERT/UPDATE/DELETE トリガーで自動更新。集計クエリの応答時間 90%削減。 |
| 2026-04-17 | 8.28 | **AIレビュー要約（Claude Haiku）**: facility/[slug] 詳細ページに口コミ要約セクション追加。10件以上の口コミがある施設で「この施設は○○に定評があり...」の自動要約を表示。ReviewSummaryコンポーネント（`/api/ai/review-summary` 経由、Claude claude-haiku-4-5-20251001使用）。 |
| 2026-04-08 | 8.11 | **SEO基盤完成 + 本番運用基盤 + マニュアル整合監査**: (1) **致命バグ修正**: sitemap.xml が `https://www.carelink-jp.com\n` で壊れていた根本原因を駆除。Vercel環境変数 `NEXT_PUBLIC_BASE_URL` の末尾改行+wwwが原因。`src/lib/constants.ts` に `normalizeSiteUrl()` 防御層追加（trim+apex強制）+ `sitemap.ts` を `force-dynamic` 化（CDN静的キャッシュ完全回避）。(2) **47都道府県固有SEOコンテンツ**: `src/data/prefecture-seo.ts` 新規（47県×{intro 200-300字+highlights 4項目+3FAQ}、事実ベース）。`area_seo_contents` テーブルに seed 済みのジェネリックテンプレを上書きする「ハードコード優先」ロジックに変更。(3) **3700+ページ生成器SEO**: `src/lib/seo-snippets.ts` 新規（businessTypeContext 8業種 + `generatePrefTypeContent()` / `generateCityContent()` / `generateCityTypeContent()` の3レイヤー生成器）。pref×type 376P / city 283P / city×type 数千Pすべてに事実ベースの固有テキスト・H2・FAQを配信。(4) **ブログ記事 20→51本**: 医療8/美容10/福祉5/エリア特集4/求人3を追加（`src/data/articles.ts`）。(5) **30症状ページSEO強化**: `src/data/symptom-seo.ts` 新規（30症状×{intro+causes+treatments+selfCare+3FAQ}）。`/symptom/[slug]` を Next.js 15準拠の Promise<{slug}> に修正、FAQPage+BreadcrumbList JSON-LD追加、症状解説・原因・治療・FAQをサーバーレンダリング。(6) **業種別グローバル8ページ新設**: `/type/[typeSlug]` 新ルート（generateStaticParams=8業種）。47県への内部リンク+人気施設12件+ItemList+FAQPage JSON-LD。sitemap.ts に `businessTypeTopPages` 追加。(7) **本番運用基盤**: `/api/health`（DB疎通+応答時間+commit hash返却、UptimeRobot等の外形監視用） / `/api/sentry-check`（Sentry動作確認、`?fire=1&token=...` でテストエラー実発火）新設。`docs/LAUNCH-CHECKLIST.md` 新規（Phase A: GSC/UptimeRobot/Sentry, Phase B: Stripe本番化5ステップ, Phase C: 実機E2E, Phase D: 法令, Phase E: ソフトローンチ）。(8) **マニュアル整合監査（35回目+36回目の二次監査）**: API 27→**32**, migration 26→27, コンポーネント 78→80, ブログ 5→51 に修正。`/jobs` `/jobs/[id]` `/type/[typeSlug]` をページ表に追加（特に `/jobs` は v8.10時点で「削除済み」と誤記載されていたが実装存在を確認し復活）。`/api/og` 削除済みの記述も実態と異なるため訂正（実際は存在し動的生成中）。§4.1環境変数表に `NEXT_PUBLIC_GSC_VERIFICATION_APEX` `SENTRY_TEST_TOKEN` 追加。§4.3 .env.exampleコード例も同期。§6.1 Phase11テーブル一覧に `facility_jobs` テーブル+`facility_profiles.is_seed` ALTER追加。§6.1.1 Migration一覧に `20260407000001_seed_flag_and_jobs.sql` 追加。§3 ディレクトリ構成に `scripts/` (seed-facilities.mjs/cleanup-seed.mjs) と `LAUNCH-CHECKLIST.md` 追加。 |
| 2026-04-07 | 8.10 | **インフラ全面有効化+Stripe決済バグ修正+法務改訂**: (1) Vercel環境変数追加=`NEXT_PUBLIC_SENTRY_DSN`/`UPSTASH_REDIS_REST_URL`/`UPSTASH_REDIS_REST_TOKEN`/`STRIPE_SECRET_KEY`/`STRIPE_WEBHOOK_SECRET`(テストモード)/`NEXT_PUBLIC_LINE_CHANNEL_ID`/`LINE_CHANNEL_SECRET`（Sentry・Upstash分散レート制限・Stripe・LINE OAuth稼働開始）。(2) Stripe決済バグ3件修正=金額改ざん(client送信amount廃止→サーバー側`bookings.total_price ?? menu.price`で決定)/IDOR(`booking.user_id !== user.id`で403)/冪等性(`stripe_events`テーブルでevent.id重複INSERT拒否)。(3) `payment_intent.payment_failed`ハンドラ追加→`payment_status='failed'`更新。Stripe Webhook受信イベント7件（checkout.session.completed + subscription 3種 + invoice 2種 + payment_intent.payment_failed）。(4) DB migration `20260407000002_stripe_events.sql`=`stripe_events`テーブル新設+`bookings.payment_status` CHECK制約に`'failed'`追加。(5) privacy/legal大改訂=個情法28条対応（業務委託先5社明示/外国第三者提供）、安全管理措置4分類、開示等請求、苦情窓口（個人情報保護委員会）、特商法必須項目補完（販売価格/支払時期/申込有効期限/返品特約/動作環境）。(6) sitemap.ts に `/compare` `/legal` `/register` 追加。(7) layout.tsx に Twitter Card(summary_large_image)追加。(8) robots.ts Sitemap URL を apex ドメイン(`https://carelink-jp.com/sitemap.xml`)に修正。migration 25→26ファイル（`combined_phase2_to_6.sql`含む）、外部サービス設定状況を全項目✅に更新 |
