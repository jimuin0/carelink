...
| Jest + CI/CD | ✅ 設定済み | 71テスト（8スイート）、GitHub Actions CI |
...
# Upstash Redis（省略時: in-memoryフォールバック）
...
- **SSL証明書**: 自動生成済み
- **残作業**: Vercel環境変数 `NEXT_PUBLIC_BASE_URL` 更新、Supabase Auth Site URL 更新
...
### 20.2 テストスイート（71テスト / 8スイート）

| ファイル | テスト数 | 内容 |
|---------|:-------:|------|
| `src/lib/__tests__/validations.test.ts` | 14 | step1/step2/step3スキーマ + formatPhone |
| `src/lib/__tests__/validations-booking.test.ts` | 10 | bookingSchema |
| `src/lib/__tests__/validations-inquiry.test.ts` | 11 | inquirySchema電話番号バリデーション |
| `src/lib/__tests__/constants.test.ts` | 11 | UUID_REGEX、prefectures 47、businessTypes、regionGroups、dayOrder/dayLabels |
| `src/lib/__tests__/csrf.test.ts` | 5 | CSRF検証 |
| `src/lib/__tests__/rate-limit.test.ts` | 3 | in-memoryフォールバック |
| `src/lib/__tests__/rate-limit-advanced.test.ts` | 4 | window expiry、limit=0、empty IP、checkRateLimitフォールバック |
| `src/components/__tests__/Spinner.test.tsx` | 3 | Spinnerコンポーネント |
...
| NEXT_PUBLIC_BASE_URL未更新 | Vercel環境変数をcarelink-jp.comに更新が必要 |
...
| 高 | 環境変数更新 | NEXT_PUBLIC_BASE_URL + Supabase Auth Site URL をcarelink-jp.comに |
...
| 日付 | バージョン | 内容 |
|------|-----------|------|
| 2026-04-01 | 7.3(MANUAL) | **MANUAL v7.3更新**: v7.0〜v7.3の全内容反映（Sentry・Upstash Redis・Jest 71テスト・CI/CD・OG動的化・DBインデックス・Web Push・カスタムドメイン）。外部サービス設定状況更新、制限事項から完了済み項目削除、テストセクション全面書き直し、品質監査20回230件+に更新 |
