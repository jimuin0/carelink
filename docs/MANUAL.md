...
|---------|:------------:|:----------:|:------:|:---:|
| モバイル（Slow 4G） | 87 | 93 | 92 | 100 |
| デスクトップ | 95 | - | - | - |

> v7.5→v7.6でモバイルスコア 62→87（+25点）に改善。
...
| LCP (Largest Contentful Paint) | 7.3s | **3.8s** | -48% |
| SI (Speed Index) | 5.8s | **3.9s** | -33% |
| TBT (Total Blocking Time) | 0ms | 10ms | ≒同等 |
...
- **hero画像quality最適化**: Next.js Image `quality={60}`（グラデーションオーバーレイ下で十分な品質）。AVIF自動変換+preloadリンク維持
...
LCP（Largest Contentful Paint）が3.8sで90点到達の壁。原因はSlow 4G（1.6Mbps）の帯域幅制約:
- HTML(15KB gz) + CSS(15KB gz) + Image(20KB AVIF) + JS(217KB gz) = 合計267KB
- 267KB / 200KB/s = 1.3秒（理想値）+ TCP slow start + RTT×数回 = 最低2-3秒

90点以上を目指す場合の候補:
- SearchFooterのSEOリンク群をHTML量削減（`<details>`初期閉じ等）
- Supabase JSチャンク51KB(gz)の遅延読み込み
- ただし効果は限定的（±3-5点程度）
...
| ファイル | 変換前 | 変換後 | 削減率 |
|---------|:------:|:------:|:------:|
| hero.webp | 165KB（JPEG偽装） | 71KB（本物WebP） | 57% |
| cta.webp | 83KB（JPEG偽装） | 28KB（本物WebP） | 66% |

> 元ファイルはWebP拡張子だがJPEGバイナリだったため、cwebpで本物のWebP形式に変換。
...
| 23(v7.6) | 3件 | **PageSpeed 62→87**: システムフォント化(191KB CSS除去)・below-fold遅延ロード(HTML 219KB→97KB)・hero quality 60 |
...
| 日付 | バージョン | 内容 |
|------|-----------|------|
| 2026-04-04 | 7.6 | **PageSpeed大幅改善（62→87、+25点）**: システムフォント化（next/font Noto Sans JP→システムフォント、191KBレンダリングブロックCSS除去）、below-fold遅延ロード（HomeBelowFold ssr:false切り出し、HTML 219KB→97KB）、hero画像quality 60。FCP 5.1s→0.9s(-82%)、LCP 7.3s→3.8s(-48%)、レンダリングブロック 5,760ms→100ms(-98%) |
